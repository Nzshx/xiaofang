...
async def download_reports(payload: dict = Body(...)):
    ...
    try:
        ...
        # ===== 表头 =====
        ws.append(["序号", "验收图片", "隐患说明", "专业", "规范条款索引/已核对原文", "分析时间"])
        ...
        for doc in cursor:
            ...
            for i, hz in enumerate(hazards):
                ...
                vio_texts = []
                for ref in hz.get("regulation_refs", []) or []:
                    standard = ref.get("standard", "")
                    clause = ref.get("clause", "")
                    status = ref.get("verification_status", "待专业核对")
                    if standard or clause:
                        vio_texts.append(f"{standard} {clause}（{status}）".strip())
                for v in hz.get("violations", []):
                    std = v.get("standard", "")
                    clause = v.get("clause", "")
                    content = v.get("content", "")
                    vio_texts.append(f"{std} {clause}（已核对原文）\n{content}".strip())
                ws.cell(r, 5, "\n".join(vio_texts))
