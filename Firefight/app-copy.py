...
def _build_fusion_prompt(c1: str, c2: str, kb_text: str) -> str:
    ...
    return (
        f"请对比以下两张图片：\n"
        "1) 第一张是待检测现场图片（Scene）。\n"
        "2) 第二张是知识库提供的标准参考图片（Standard）。\n\n"
        f"任务：仅就隐患类型「{c12}」判断该隐患在现场是否存在。\n"
        ...
        "}\n\n"
        "【重要硬性约束】\n"
        "1) regulation_refs 只填写知识库证据区明确出现的规范名称和条款号；不得补写条文内容。\n"
        "2) 知识库仅有条款号时，violations 必须为 []。\n"
        "3) 只有知识库证据区已提供完整原文时，violations 才可填写；content 必须逐字复制原文，"
        "每项格式为 {\"standard\":\"规范名称\",\"clause\":\"第X条\",\"content\":\"原文\"}。\n"
        "4) 只输出 JSON，不要包含任何解释或代码块标记。\n\n"
        "【知识库证据区】（可能只有条款索引，并不等于规范原文）\n"
...
async def _doubao_fusion(
    ...
    # 条款索引和规范原文分开处理：索引可先输出，原文必须在知识库中逐字核对。
