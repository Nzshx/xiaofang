...
from multi_drawing_pipeline.stages.fire_route_core import stage_07_connector_metric_closure as _stage07
from multi_drawing_pipeline.stages.fire_route_core import stage_08_semantic_rgcn as _stage08
from multi_drawing_pipeline.stages.fire_route_core import stage_09_dual_graph_planning as _stage09

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/last/io_utils.py
# -----------------------------------------------------------------------------
def _build_s10_io():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/last/io_utils.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.last.io_utils'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.last'
    import json
...
_s10_io = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.last.io_utils',
    _build_s10_io(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/last/vector_free_space.py
# -----------------------------------------------------------------------------
def _build_s10_vector_free():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/last/vector_free_space.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.last.vector_free_space'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.last'
    ...
    from multi_drawing_pipeline.stages.fire_route_core.last.io_utils import read_json, text
...
_s10_vector_free = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.last.vector_free_space',
    _build_s10_vector_free(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/last/physical_graph.py
# -----------------------------------------------------------------------------
def _build_s10_physical_graph():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/last/physical_graph.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.last.physical_graph'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.last'
    import heapq
    import math
...
_s10_physical_graph = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.last.physical_graph',
    _build_s10_physical_graph(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/last/control_plane.py
# -----------------------------------------------------------------------------
def _build_s10_control():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/last/control_plane.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.last.control_plane'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.last'
    from pathlib import Path
    from typing import Any, Iterable
    from multi_drawing_pipeline.stages.fire_route_core.last.io_utils import point_from_mapping, read_json, text, write_json
...
_s10_control = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.last.control_plane',
    _build_s10_control(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/last/forwarding_plane.py
# -----------------------------------------------------------------------------
def _build_s10_forwarding():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/last/forwarding_plane.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.last.forwarding_plane'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.last'
    ...
    from multi_drawing_pipeline.stages.fire_route_core.last.io_utils import text
    from multi_drawing_pipeline.stages.fire_route_core.last.physical_graph import FloorPhysicalGraph, PhysicalPath, RefinedPhysicalGraph
    from multi_drawing_pipeline.stages.fire_route_core.last.vector_free_space import VectorFreeSpaceIndex
...
_s10_forwarding = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.last.forwarding_plane',
    _build_s10_forwarding(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/last/acceptance_report.py
# -----------------------------------------------------------------------------
def _build_s10_acceptance():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/last/acceptance_report.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.last.acceptance_report'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.last'
    from pathlib import Path
    from typing import Any
    from multi_drawing_pipeline.stages.fire_route_core.last.io_utils import read_json, text, write_json
...
_s10_acceptance = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.last.acceptance_report',
    _build_s10_acceptance(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/last/annotated_output.py
# -----------------------------------------------------------------------------
def _build_s10_annotated():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/last/annotated_output.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.last.annotated_output'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.last'
    import csv
    import json
    import math
    ...
    from multi_drawing_pipeline.stages.fire_route_core.last.io_utils import write_json
    from shapely.geometry import shape
    ...
    def _draw_polygon_geojson(msp: Any, paths: Iterable[Path | str], layer: str, color: int) -> int:
        drawn = 0

        def draw_geometry(geometry: Any) -> None:
            nonlocal drawn
            ...
            if geometry.geom_type == 'Polygon':
                for ring in [geometry.exterior, *geometry.interiors]:
                    ...
                    if len(coordinates) >= 4:
                        msp.add_lwpolyline(coordinates, close=True, dxfattribs={'layer': layer, 'color': color, 'lineweight': 35})
                        drawn += 1
                return
        ...
        for value in paths:
            ...
            for feature in payload.get('features', []) or []:
                if feature.get('geometry'):
                    draw_geometry(shape(feature['geometry']))
        return drawn

    def write_annotated_route_dxf(source_dxf: Path | str | None, output_dxf: Path | str, visit_plan: dict[str, Any], forwarding: dict[str, Any], *, original_obstacle_paths: Iterable[Path | str]=(), approved_repair_path: Path | None=None, include_review_annotations: bool=False) -> Path:
        ...
        msp = doc.modelspace()
        # The delivery DXF is intentionally clean: the fused architectural
        # drawing already contains the migrated discrete inspection objects,
        # so only route polylines are added.  Review-only obstacle outlines,
        # target frames/order labels and virtual-continuation labels remain in
        # JSON/CSV/reports and can be enabled only for a separate review DXF.
        if include_review_annotations:
            obstacle_paths = list(original_obstacle_paths)
            if obstacle_paths:
                _layer(doc, 'AI_RECOGNIZED_OBSTACLE', 1, 35)
                _draw_polygon_geojson(msp, obstacle_paths, 'AI_RECOGNIZED_OBSTACLE', 1)
            if approved_repair_path:
                _layer(doc, 'AI_WALL_TOPOLOGY_REPAIR', 4, 35)
                _draw_polygon_geojson(msp, [approved_repair_path], 'AI_WALL_TOPOLOGY_REPAIR', 4)
        ...
        for floor_id, floor_route in sorted((forwarding.get('floors') or {}).items()):
            ...
            if not include_review_annotations:
                continue
...
_s10_annotated = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.last.annotated_output',
    _build_s10_annotated(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/last/pipeline.py
# -----------------------------------------------------------------------------
def _build_s10_pipeline():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/last/pipeline.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.last.pipeline'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.last'
    import argparse
    import time
    from pathlib import Path
    from typing import Any, Iterable
    from multi_drawing_pipeline.stages.fire_route_core.last.acceptance_report import write_acceptance_reports
    from multi_drawing_pipeline.stages.fire_route_core.last.annotated_output import forwarding_to_geojson, write_annotated_route_dxf, write_event_csvs, write_inspection_visit_order
    from multi_drawing_pipeline.stages.fire_route_core.last.control_plane import build_locked_visit_plan
    from multi_drawing_pipeline.stages.fire_route_core.last.forwarding_plane import forward_locked_visit_plan
    from multi_drawing_pipeline.stages.fire_route_core.last.io_utils import read_json, text, write_json
    from multi_drawing_pipeline.stages.fire_route_core.last.physical_graph import RefinedPhysicalGraph
    from multi_drawing_pipeline.stages.fire_route_core.last.vector_free_space import VectorFreeSpaceIndex
    ...
    def build_last_inspection_route(run_dir: Path | str, optimized_order_path: Path | str, candidates_path: Path | str, effective_free_areas_path: Path | str, *, output_dir: Path | str | None=None, refined_graph_path: Path | str | None=None, source_dxf: Path | str | None=None, floors: Iterable[str] | None=None, route_cache_path: Path | str | None=None, max_grid_cells: int=1500000, write_dxf: bool=True) -> dict[str, Any]:
        ...
        # Stage 06 hard obstacles/topology repairs and Stage 09 free-space
        # certification are the active safety gates. Door-Portal repair and the
        # independent raw-CAD collision layer are intentionally disabled.
        from multi_drawing_pipeline.stages.fire_route_core import navigation_obstacles as hard_walls
        approved = hard_walls.approved_manifest(run)
        original_obstacle_paths: list[Path] = []
        repair_path = None
        if approved:
            original_obstacle_paths = [Path(row['path']).resolve() for row in approved['sources'] if not row['is_repair']]
            repair_sources = [Path(row['path']).resolve() for row in approved['sources'] if row['is_repair']]
            repair_path = repair_sources[0] if repair_sources else None
        else:
            pipeline_path = run / 'pipeline_summary.json'
            if pipeline_path.is_file():
                obstacle_values = (read_json(pipeline_path).get('obstacle_recognition') or {}).get('union_geojsons', []) or []
                original_obstacle_paths = [Path(value).resolve() for value in obstacle_values]
        ...
        if write_dxf:
            try:
                ...
                preferred_path = output / f'{stem}_final_safe_route.dxf'
                try:
                    dxf_path = write_annotated_route_dxf(source, preferred_path, visit_plan, forwarding, original_obstacle_paths=original_obstacle_paths, approved_repair_path=repair_path, include_review_annotations=False)
                except PermissionError:
                    version = time.strftime('%Y%m%d_%H%M%S')
                    dxf_path = write_annotated_route_dxf(source, output / f'{stem}_final_safe_route_{version}.dxf', visit_plan, forwarding, original_obstacle_paths=original_obstacle_paths, approved_repair_path=repair_path, include_review_annotations=False)
...
_s10_pipeline = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.last.pipeline',
    _build_s10_pipeline(),
    aliases=(),
)
