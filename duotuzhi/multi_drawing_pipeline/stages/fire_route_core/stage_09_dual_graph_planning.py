...
# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/pseudo_label_route_solver.py
# -----------------------------------------------------------------------------
def _build_s09_pseudo_solver():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/semantic/pseudo_label_route_solver.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic.pseudo_label_route_solver'
        ...
        def __init__(self, floor_id: str, nodes: Sequence[Mapping[str, Any]], edges: Sequence[Mapping[str, Any]]) -> None:
            ...
            for index, edge in enumerate(self.edges):
                ...
                if left not in self.nodes or right not in self.nodes:
                    continue
                length = max(0.0, _finite(edge.get('routing_cost'), _finite(edge.get('length'), 0.0)))
...
_s09_pseudo_solver = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.semantic.pseudo_label_route_solver',
    _build_s09_pseudo_solver(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/chain_route_planner.py
# -----------------------------------------------------------------------------
def _build_s09_chain():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/semantic/chain_route_planner.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic.chain_route_planner'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic'
    ...
    from multi_drawing_pipeline.stages.fire_route_core.semantic.physical_metric_closure import PhysicalMetricClosure
    from multi_drawing_pipeline.stages.fire_route_core.semantic.pseudo_label_route_solver import PhysicalFloorGraph
    EPS = 1e-09
...
_s09_chain = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.semantic.chain_route_planner',
    _build_s09_chain(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/dual_graph/planner.py
# -----------------------------------------------------------------------------
def _build_s09_planner():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/dual_graph/planner.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.dual_graph.planner'
    ...
    from multi_drawing_pipeline.stages.fire_route_core.last.pipeline import build_last_inspection_route
    ...
    from multi_drawing_pipeline.stages.fire_route_core.semantic.chain_route_planner import RecommendationView, select_exact_rule_targets
...
_s09_planner = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.dual_graph.planner',
    _build_s09_planner(),
    aliases=(),
)
