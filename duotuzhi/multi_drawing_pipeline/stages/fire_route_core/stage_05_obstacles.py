...
# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/obstacle_recognition_door_mask_fixed.py
# -----------------------------------------------------------------------------
def _build_s05_obstacles():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/obstacle_recognition_door_mask_fixed.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.obstacle_recognition_door_mask_fixed'
    ...
    # Bounded parallelism for layer-LLM batches.  Override with the
    # DUOTUZHI_LAYER_LLM_CONCURRENCY environment variable; 1 restores the
    # original strictly serial behaviour.
    try:
        LAYER_LLM_MAX_CONCURRENCY = max(1, int(os.getenv('DUOTUZHI_LAYER_LLM_CONCURRENCY', '3') or '3'))
    except ValueError:
        LAYER_LLM_MAX_CONCURRENCY = 3
    ...
    def llm_runtime_config() -> tuple[str, str, str]:
        ...
        # A generic OPENAI_API_KEY may target another provider and must not
        # override the project-local DeepSeek credential.
        api_key = os.getenv('DEEPSEEK_API_KEY') or defaults.get('api_key', '') or os.getenv('OPENAI_API_KEY')
    ...
    def classify_layers_by_llm(rows: list[dict[str, str]], output_dir: Path) -> dict[str, dict[str, Any]]:
        ...
        def merge_raw_decisions(raw_decisions: list[dict[str, Any]]) -> None:
        ...
        batches = [pending_summaries[start:start + batch_size] for start in range(0, len(pending_summaries), batch_size)]
        max_workers = min(LAYER_LLM_MAX_CONCURRENCY, len(batches))
        if max_workers <= 1:
            for batch in batches:
                ...
                merge_raw_decisions(raw_decisions)
                write_layer_llm_output(cache_path, model=model, base_url=base_url, summaries=summaries, decisions=decisions, complete=False)
        else:
            from concurrent.futures import ThreadPoolExecutor, as_completed
            with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix='obstacle-layer-llm') as executor:
                future_to_batch = {
                    executor.submit(request_layer_llm_resilient, batch, api_key=api_key, base_url=base_url, model=model): batch
                    for batch in batches
                }
                try:
                    for future in as_completed(future_to_batch):
                        raw_decisions = future.result()
                        merge_raw_decisions(raw_decisions)
                        # Persist after every finished batch so a later failure
                        # keeps the completed work in the on-disk cache.
                        write_layer_llm_output(cache_path, model=model, base_url=base_url, summaries=summaries, decisions=decisions, complete=False)
                except LLM_REQUEST_ERRORS as exc:
    ...
    # Candidate-layer blocks: cross-layer parallel pairs; any arc vetoes the block.
    import math
    import csv
    ...
    RULE_VERSION = "candidate-insert-parallel-pairs-v2-no-arcs"

    COMPONENT_RULE_VERSION = "mixed-layer-components-door-85-95deg-600-1150mm-v1"

    def layer_has_obstacle_attribute(layer: str) -> bool:
        return bool(re.search(r'墙|窗|柱|砌体|混凝土|WALL|WINDOW|CURTAIN|GLAZ|GLZ|COLUMN|PILLAR|MASONRY|CONCRETE|(?:^|[^A-Z])WIN(?:$|[^A-Z])', layer, re.I))

    def layer_has_door_attribute(layer: str) -> bool:
        return bool(re.search(r'门|DOOR', layer, re.I))

    def component_review_layers(decisions: dict[str, dict[str, Any]]) -> set[str]:
        # Keep the original LLM decision; recovery is a separate geometry branch.
        return {layer for layer, decision in decisions.items()
                if layer_has_obstacle_attribute(layer)
                and (decision.get('role') == 'not_obstacle' or layer_has_door_attribute(layer))
                and not layer_is_forced_non_obstacle(layer)}

    def drawing_mm_per_unit(doc: Any) -> float | None:
        from ezdxf import units
        try:
            return float(units.conversion_factor(int(doc.units), units.MM)) if doc.units else None
        except (ValueError, TypeError):
            return None

    def strict_door_arc(entity: Any, semantic_layer: str, mm_per_unit: float | None) -> dict[str, Any] | None:
        """Rule 2: actual ARC, door layer, 85..95 degrees, 600..1150 mm."""
        if entity.dxftype() != 'ARC' or not layer_has_door_attribute(semantic_layer) or mm_per_unit is None:
            return None
        sweep = (float(entity.dxf.end_angle) - float(entity.dxf.start_angle)) % 360.0
        radius_mm = float(entity.dxf.radius) * mm_per_unit
        if not (85.0 - 1e-8 <= sweep <= 95.0 + 1e-8 and 600.0 - 1e-6 <= radius_mm <= 1150.0 + 1e-6):
            return None
        return {'sweep_degrees': sweep, 'radius_mm': radius_mm,
                'radius_drawing_units': float(entity.dxf.radius),
                'center': [float(entity.dxf.center.x), float(entity.dxf.center.y)],
                'start_angle': float(entity.dxf.start_angle), 'end_angle': float(entity.dxf.end_angle),
                'semantic_layer': semantic_layer, 'arc_layer': str(entity.dxf.layer)}

    def review_layer_components(doc: Any, regions: list[dict[str, Any]],
                                decisions: dict[str, dict[str, Any]], api: Any, config: Any,
                                max_depth: int = 10) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        """Recover mixed/LLM-negative structural layers without promoting an entire layer.

        INSERTs are assessed in world coordinates. Loose straight strokes are
        grouped per floor/layer; insufficient geometry remains unclassified.
        No rectangular door mask is subtracted from structural geometry.
        """
        from ezdxf import bbox as ezbbox
        review_layers = component_review_layers(decisions)
        mm_per_unit = drawing_mm_per_unit(doc)
        records, doors, owned, output = [], [], [], []
        loose: dict[tuple[str, str], list[Any]] = defaultdict(list)
        loose_regions = {}

        @lru_cache(maxsize=None)
        def contains_target(name: str, ancestors: tuple[str, ...] = ()) -> bool:
            if name in ancestors or len(ancestors) >= max_depth:
                return False
            block = doc.blocks.get(name)
            return bool(block is not None and any(
                str(e.dxf.layer) in review_layers or layer_has_door_attribute(str(e.dxf.layer))
                or (e.dxftype() == 'INSERT' and contains_target(str(e.dxf.name), ancestors + (name,)))
                for e in block))

        def expand(entity: Any, depth: int, names: tuple[str, ...]) -> tuple[list[Any], bool]:
            if entity.dxftype() != 'INSERT':
                return [entity], True
            name = str(entity.dxf.name)
            if depth >= max_depth or name in names:
                return [], False
            skipped = []
            leaves, complete = [], True
            try:
                for child in entity.virtual_entities(skipped_entity_callback=lambda e, reason: skipped.append(reason)):
                    instances = list(child.multi_insert()) if child.dxftype() == 'INSERT' and child.mcount > 1 else [child]
                    for instance in instances:
                        part, ok = expand(instance, depth + 1, names + (name,))
                        leaves.extend(part)
                        complete = complete and ok
            except Exception:
                complete = False
            return leaves, complete and not skipped

        def append_faces(geometries: list[Any], layer: str, path: str, handle: str,
                         kind: str, record: dict[str, Any]) -> None:
            geometries = [part for geometry in geometries for part in api.polygon_parts(geometry)
                          if not part.is_empty and part.area > config.min_area]
            if not geometries:
                return
            geom = api.valid_polygonal_union(geometries, context='mixed_component:' + path)
            row = {'object_id': 'COMPONENT::' + path, 'handle': handle, 'layer': layer,
                   'entity_type': kind, 'geometry_kind': 'component_geometry'}
            for region in regions:
                if geom.is_empty or not geom.intersects(region['polygon']):
                    continue
                for part in api.polygon_parts(geom.intersection(region['polygon'])):
                    if part.area <= config.min_area:
                        continue
                    api.add_obstacle(output, row=row, region=region, geom=part,
                        obstacle_type='filled_obstacle', reason='mixed_layer_component_geometry_after_strict_door_check',
                        confidence=0.9, semantic_evidence='component_review; original_role=' + str(decisions.get(layer, {}).get('role')),
                        geometry_evidence='closed_faces_or_multiple_parallel_pairs',
                        topology_evidence='rule=' + COMPONENT_RULE_VERSION,
                        fusion_decision='door_not_confirmed_and_structural_geometry_confirmed')
                    record['obstacle_count'] = record.get('obstacle_count', 0) + 1
                    record['status'] = 'obstacle_geometry_confirmed'

        def visit(entity: Any, depth: int, names: tuple[str, ...], path: str, inherited: str = '') -> None:
            kind = entity.dxftype()
            raw_layer = str(entity.dxf.layer)
            layer = inherited if raw_layer == '0' and inherited else raw_layer
            is_target = layer in review_layers or layer_has_door_attribute(layer)
            if kind == 'INSERT' and not is_target:
                name = str(entity.dxf.name)
                if depth >= max_depth or name in names or not contains_target(name):
                    return
                try:
                    for index, child in enumerate(entity.virtual_entities()):
                        instances = list(child.multi_insert()) if child.dxftype() == 'INSERT' and child.mcount > 1 else [child]
                        for j, instance in enumerate(instances):
                            visit(instance, depth + 1, names + (name,), f'{path}/{index}:{j}', layer)
                except Exception:
                    records.append({'instance_path': path, 'layer': layer, 'status': 'incomplete_expansion_requires_review'})
                return
            if not is_target or kind not in {'INSERT', 'ARC', 'LINE', 'LWPOLYLINE', 'POLYLINE', 'HATCH', 'CIRCLE', 'SOLID'}:
                return
            leaves, complete = expand(entity, depth, names)
            handle = str(entity.dxf.get('handle', ''))
            record = {'instance_path': path, 'handle': handle, 'layer': layer, 'entity_type': kind,
                      'block_name': str(entity.dxf.name) if kind == 'INSERT' else '',
                      'original_role': decisions.get(layer, {}).get('role', 'unknown'),
                      'expansion_complete': complete, 'status': 'geometry_insufficient_requires_review', 'obstacle_count': 0}
            records.append(record)
            bounds = None
            # Do not ask ezdxf's bbox traversal to expand cyclic INSERTs again.
            try:
                ext = ezbbox.extents(leaves, fast=True)
                if ext.has_data:
                    bounds = [float(ext.extmin.x), float(ext.extmin.y), float(ext.extmax.x), float(ext.extmax.y)]
            except Exception:
                pass
            if layer in review_layers:
                owned.append({'layer': layer, 'handle': handle, 'block_name': record['block_name'], 'bbox': bounds})
            if not complete:
                record['status'] = 'incomplete_expansion_requires_review'
                return
            matches = [match for leaf in leaves if (match := strict_door_arc(leaf, layer, mm_per_unit))]
            if kind in {'ARC', 'INSERT'} and matches:
                record['status'] = 'door_confirmed_by_rule_2'
                record['door_arc_evidence'] = matches
                doors.append({**record, 'bbox': bounds})
                if layer not in review_layers:
                    owned.append({'layer': layer, 'handle': handle, 'block_name': record['block_name'], 'bbox': bounds})
                return
            if mm_per_unit is None and any(e.dxftype() == 'ARC' for e in leaves):
                record['door_check_note'] = 'drawing_units_unspecified_cannot_confirm_radius_mm'
            if layer not in review_layers:
                return
            lines, faces = [], []
            for leaf in leaves:
                parts = _straight_segments(leaf)
                lines.extend(parts)
                if leaf.dxftype() in {'LWPOLYLINE', 'POLYLINE'} and not _has_arc(leaf):
                    closed = leaf.closed if leaf.dxftype() == 'LWPOLYLINE' else leaf.is_closed
                    if closed and parts:
                        faces.extend(api.polygon_parts(api.valid_polygonal_union(list(polygonize(parts)), context='mixed_closed_polyline')))
            unique = {tuple(sorted(tuple(round(v, 7) for v in p) for p in line.coords)): line for line in lines}
            lines = list(unique.values())
            if kind == 'INSERT':
                bands = _parallel_bands(lines, api, config)
                record['parallel_pair_count'] = len(bands)
                if len(bands) >= 2:
                    faces.extend(bands)
                if faces:
                    append_faces(faces, layer, path, handle, kind, record)
            else:
                if faces:
                    append_faces(faces, layer, path, handle, kind, record)
                else:
                    for region in regions:
                        for line in lines:
                            if line.intersects(region['polygon']):
                                key = (region['full_region_id'], layer)
                                loose[key].append(line)
                                loose_regions[key] = region

        for index, entity in enumerate(doc.modelspace()):
            instances = list(entity.multi_insert()) if entity.dxftype() == 'INSERT' and entity.mcount > 1 else [entity]
            for j, instance in enumerate(instances):
                visit(instance, 0, (), f'{entity.dxf.handle or index}:{j}')
        for (region_id, layer), lines in loose.items():
            lines = list({tuple(sorted(tuple(round(v, 7) for v in p) for p in line.coords)): line for line in lines}.values())
            bands = _parallel_bands(lines, api, config)
            # Isolated pairs from unrelated objects must not satisfy the two-pair rule.
            tree = STRtree(bands) if bands else None
            accepted = [band for i, band in enumerate(bands) if any(int(j) != i and band.distance(bands[int(j)]) < 1e-6
                        for j in tree.query(band.buffer(1e-6)))] if tree is not None else []
            record = {'instance_path': 'loose:' + region_id + ':' + layer, 'layer': layer,
                      'entity_type': 'LINE_GROUP', 'parallel_pair_count': len(bands),
                      'status': 'geometry_insufficient_requires_review', 'obstacle_count': 0}
            records.append(record)
            append_faces([g.intersection(loose_regions[(region_id, layer)]['polygon']) for g in accepted],
                         layer, record['instance_path'], '', 'LINE_GROUP', record)
        return output, {'rule_version': COMPONENT_RULE_VERSION, 'review_layers': sorted(review_layers),
                        'drawing_insunits': int(doc.units), 'mm_per_drawing_unit': mm_per_unit,
                        'door_rule': {'entity_types': ['ARC', 'INSERT_containing_ARC'], 'layer_terms': ['门', 'DOOR'],
                                      'angle_degrees': [85, 95], 'radius_mm': [600, 1150]},
                        'confirmed_door_count': len(doors), 'obstacle_count': len(output),
                        'status_counts': dict(Counter(r['status'] for r in records)),
                        'confirmed_doors': doors, 'owned_components': owned, 'components': records}

    def exclude_reviewed_component_rows(rows: list[dict[str, str]], audit: dict[str, Any]) -> list[dict[str, str]]:
        """Prevent confirmed doors/recovered blocks leaking through other obstacle branches."""
        layers = set(audit['review_layers'])
        handles = {r['handle'] for r in audit['owned_components'] if r['handle']}
        owners = [r for r in audit['owned_components'] if r['block_name'] and r['bbox']]
        boxes = [box(*r['bbox']).buffer(1e-5) for r in owners]
        tree = STRtree(boxes) if boxes else None
        filtered = []
        for row in rows:
            if row.get('layer') in layers or row.get('handle') in handles:
                continue
            bounds = bbox_from_row(row)
            # Horizontal/vertical leaf segments have degenerate boxes too.
            if bounds is None:
                values = [safe_float(row.get(k)) for k in ('bbox_minx', 'bbox_miny', 'bbox_maxx', 'bbox_maxy')]
                bounds = tuple(values) if all(v is not None for v in values) else None
            try:
                names = set(json.loads(str(row.get('block_path') or '[]')))
            except (ValueError, TypeError):
                names = {str(row.get('parent_block_name') or '')}
            if tree is not None and bounds is not None and names:
                envelope = box(*bounds).buffer(1e-7)
                if any(owners[int(j)]['block_name'] in names and boxes[int(j)].covers(envelope) for j in tree.query(envelope)):
                    continue
            filtered.append(row)
        return filtered


    def load_insert_semantic_rows(inventory_dir: Path) -> list[dict[str, str]]:
        """Include parent-only layers in semantic classification, not just leaves."""
        path = Path(inventory_dir) / "cad_object_inventory.csv"
        if not path.is_file():
            return []
        with path.open(encoding="utf-8-sig", newline="") as stream:
            return [dict(row) for row in csv.DictReader(stream) if row["entity_type"] == "INSERT"]


    def _has_arc(entity: Any) -> bool:
        """An arc anywhere in a candidate instance vetoes this recovery rule."""
        kind = entity.dxftype()
        if kind == "ARC":
            return True
        if kind == "ELLIPSE":
            # Non-uniform scaling converts circular arcs to elliptical arcs.
            span = abs(float(entity.dxf.end_param) - float(entity.dxf.start_param))
            return not math.isclose(span % math.tau, 0.0, abs_tol=1e-9)
        if kind == "LWPOLYLINE":
            bulges = [p[2] for p in entity.get_points("xyb")]
            closed = entity.closed
        elif kind == "POLYLINE" and entity.is_2d_polyline:
            bulges = [v.dxf.get("bulge", 0.0) for v in entity.vertices]
            closed = entity.is_closed
        else:
            return False
        return any(abs(b) > 1e-10 for b in (bulges if closed else bulges[:-1]))


    def _straight_segments(entity: Any) -> list[LineString]:
        kind = entity.dxftype()
        if kind == "LINE":
            points = [(entity.dxf.start.x, entity.dxf.start.y, 0.0),
                      (entity.dxf.end.x, entity.dxf.end.y, 0.0)]
            closed = False
        elif kind == "LWPOLYLINE":
            points = list(entity.get_points("xyb"))
            closed = entity.closed
        elif kind == "POLYLINE" and not entity.is_polygon_mesh and not entity.is_poly_face_mesh:
            points = [(v.dxf.location.x, v.dxf.location.y, v.dxf.get("bulge", 0.0))
        ...
        for a, b in zip(points, points[1:]):
            # Arc tessellation must not fabricate parallel straight-line evidence.
            if abs(a[2]) > 1e-10 or not all(math.isfinite(v) for v in (*a[:2], *b[:2])):
                continue
    ...
    def _parallel_bands(lines: list[LineString], api: Any, config: Any) -> list[Any]:
        """Pair nearest overlapping parallel neighbours on each side of a line."""
        if len(lines) < 2:
            return []
        tree = STRtree(lines)
        pairs: dict[tuple[int, int], Any] = {}
        for i, line in enumerate(lines):
            margin = line.length * config.max_wall_width_to_overlap_ratio
            minx, miny, maxx, maxy = line.bounds
            best: dict[int, tuple[float, int, Any]] = {}
            for index in tree.query(box(minx - margin, miny - margin, maxx + margin, maxy + margin)):
                j = int(index)
                if i == j:
                    continue
                pair = api.parallel_wall_pair_geometry(line, lines[j], config)
                if pair is None or pair["width"] <= max(line.length, lines[j].length) * 1e-9:
                    continue
                # Require substantial shared span rather than a coincidental crossing.
                if pair["overlap"] < min(line.length, lines[j].length) * 0.5:
                    continue
                nx, ny = pair["normal"]
                side = 1 if ((lines[j].centroid.x - line.centroid.x) * nx
                             + (lines[j].centroid.y - line.centroid.y) * ny) > 0 else -1
                item = (float(pair["width"]), j, pair["core_polygon"])
                if side not in best or item[:2] < best[side][:2]:
                    best[side] = item
            for _, j, geometry in best.values():
                pairs[min(i, j), max(i, j)] = geometry
        return list(pairs.values())


    def detect_candidate_block_obstacles(
        input_dxf: Path, regions: list[dict[str, Any]],
        decisions: dict[str, dict[str, Any]], api: Any, config: Any,
        *, doc: Any = None, existing_obstacles: list[dict[str, Any]] | None = None,
        max_depth: int = 10,
        component_audit: dict[str, Any] | None = None,
    ) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        """Read real INSERT instances; bypass selective inventory omissions.

        virtual_entities applies each instance's transformation, including nested
        instances. Instance paths use entity indices because virtual handles can be
        empty. No pairing is performed between separate candidate INSERTs.
        """
        if doc is None:
            doc = ezdxf.readfile(input_dxf)
        candidate_layers = {name for name, d in decisions.items()
                            if d.get("role") == "obstacle_candidate"}
        delegated_layers = component_review_layers(decisions)
        candidate_layers -= delegated_layers
        confirmed_door_paths = {r['instance_path'] for r in (component_audit or {}).get('confirmed_doors', [])}
        stats: Counter[str] = Counter()
        audit: list[dict[str, Any]] = []
        output: list[dict[str, Any]] = []
        known_by_floor: dict[str, list[Any]] = {}
        for item in existing_obstacles or []:
            known_by_floor.setdefault(str(item["floor_id"]), []).append(item["geometry"])
        known = {floor: api.valid_polygonal_union(geoms, context="candidate_blocks_existing")
                 for floor, geoms in known_by_floor.items()}

        @lru_cache(maxsize=None)
        def has_candidate(name: str, ancestors: tuple[str, ...] = ()) -> bool:
            if name in ancestors or len(ancestors) >= max_depth:
                return False
            block = doc.blocks.get(name)
            if block is None:
                return False
            return any(e.dxftype() == "INSERT" and (
                str(e.dxf.layer) in candidate_layers or has_candidate(str(e.dxf.name), ancestors + (name,))
            ) for e in block)

        def expand(insert: Any, path: str) -> list[Any]:
            def skipped(entity: Any, reason: str) -> None:
                stats["unsupported_entity_count"] += 1
                audit.append({"instance_path": path, "status": "expansion_skipped",
                              "entity_type": entity.dxftype(), "reason": reason})
            try:
                return list(insert.virtual_entities(skipped_entity_callback=skipped))
            except Exception as exc:
                stats["expansion_error_count"] += 1
                audit.append({"instance_path": path, "status": "expansion_error", "reason": str(exc)})
                return []

        def collect(insert: Any, depth: int, names: tuple[str, ...], path: str,
                    lines: list[LineString], layers: set[str], arcs: list[dict[str, str]]) -> bool:
            if str(insert.dxf.layer) in delegated_layers or path in confirmed_door_paths:
                return True
            name = str(insert.dxf.name)
            if depth >= max_depth or name in names:
                stats["depth_or_cycle_skip_count"] += 1
                return False
            before_errors = stats["unsupported_entity_count"] + stats["expansion_error_count"]
            children = expand(insert, path)
            complete = before_errors == stats["unsupported_entity_count"] + stats["expansion_error_count"]
            for i, child in enumerate(children):
                if child.dxftype() == "INSERT":
                    instances = list(child.multi_insert()) if child.mcount > 1 else [child]
                    for j, instance in enumerate(instances):
                        complete = collect(instance, depth + 1, names + (name,), f"{path}/{i}:{j}", lines, layers, arcs) and complete
                else:
                    if _has_arc(child):
                        arcs.append({"entity_type": child.dxftype(), "layer": str(child.dxf.layer),
                                     "entity_path": f"{path}/{i}"})
                    parts = _straight_segments(child)
                    if parts:
                        layers.add(str(child.dxf.layer))
                        lines.extend(parts)
            return complete

        def visit(insert: Any, depth: int, names: tuple[str, ...], path: str, inherited: str = "") -> None:
            if path in confirmed_door_paths:
                return
            name = str(insert.dxf.name)
            if depth >= max_depth or name in names:
                stats["depth_or_cycle_skip_count"] += 1
                return
            raw_layer = str(insert.dxf.layer)
            layer = inherited if raw_layer == "0" and inherited else raw_layer
            if layer in delegated_layers:
                return
            if layer not in candidate_layers:
                if not has_candidate(name):
                    return
                for i, child in enumerate(expand(insert, path)):
                    if child.dxftype() == "INSERT":
                        instances = list(child.multi_insert()) if child.mcount > 1 else [child]
                        for j, instance in enumerate(instances):
                            visit(instance, depth + 1, names + (name,), f"{path}/{i}:{j}", layer)
                return
            stats["candidate_instance_count"] += 1
            lines: list[LineString] = []
            layers: set[str] = set()
            arcs: list[dict[str, str]] = []
            complete = collect(insert, depth, names, path, lines, layers, arcs)
            # Repeated/reversed strokes must not satisfy the multiple-pair rule.
            unique = {tuple(sorted(tuple(round(v, 7) for v in point) for point in line.coords)): line
                      for line in lines}
            lines = list(unique.values())
            bands = _parallel_bands(lines, api, config) if complete and not arcs else []
            row_audit = {"instance_path": path, "source_handle": str(insert.dxf.get("handle", "")),
                         "block_name": name, "layer": layer, "raw_insert_layer": raw_layer,
                         "child_layers": sorted(layers), "segment_count": len(lines),
                         "parallel_pair_count": len(bands), "expansion_complete": complete,
                         "arc_entity_count": len(arcs), "arc_evidence": arcs,
                         "status": "insufficient_parallel_pairs", "obstacle_count": 0}
            audit.append(row_audit)
            if arcs:
                stats["arc_rejected_instance_count"] += 1
                row_audit["status"] = "contains_arc_possible_opening"
                return
            if not complete:
                # Unread descendants could contain arcs; do not accept a partial block.
                stats["incomplete_rejected_instance_count"] += 1
                row_audit["status"] = "incomplete_expansion_requires_review"
                return
            if len(bands) < 2:
                return
            stats["qualified_instance_count"] += 1
            geometry = api.valid_polygonal_union(bands, context=f"block_parallel:{path}")
            row = {"object_id": f"BLOCK_PARALLEL::{path}", "handle": row_audit["source_handle"],
                   "source": str(input_dxf), "layer": layer, "entity_type": "INSERT",
                   "geometry_kind": "block_parallel_line_bands", "parent_block_name": name}
            row_audit["status"] = "outside_regions_or_already_covered"
            for region in regions:
                if not geometry.intersects(region["polygon"]):
                    continue
                clipped = geometry.intersection(region["polygon"])
                for part in api.polygon_parts(clipped):
                    if part.is_empty or part.area <= 0:
                        continue
                    old = known.get(str(region["floor_id"]))
                    if old is not None and part.intersection(old).area >= part.area * 0.999999:
                        stats["duplicate_face_count"] += 1
                        continue
                    api.add_obstacle(output, row=row, region=region, geom=part,
                        obstacle_type="filled_obstacle", reason="candidate_layer_block_multiple_parallel_pairs",
                        confidence=float(decisions[layer].get("confidence") or 0.0),
                        semantic_evidence=f"candidate_layer={layer}; child_layers={sorted(layers)}; block={name}",
                        geometry_evidence=f"parallel_pairs={len(bands)}; segments={len(lines)}; overlap_bands_only",
                        topology_evidence=f"instance_path={path}; rule={RULE_VERSION}",
                        fusion_decision="accepted_by_candidate_parent_layer_and_multiple_parallel_pairs")
                    row_audit["obstacle_count"] += 1
                    row_audit["status"] = "accepted"
        for i, entity in enumerate(doc.modelspace()):
            if entity.dxftype() == "INSERT":
                instances = list(entity.multi_insert()) if entity.mcount > 1 else [entity]
                for j, instance in enumerate(instances):
                    visit(instance, 0, (), f"{entity.dxf.handle or i}:{j}")
        return output, {"rule_version": RULE_VERSION, "minimum_parallel_pairs": 2,
                        "reject_candidate_block_with_arcs": True,
                        "child_layer_filter_enabled": False, **dict(stats),
                        "accepted_instance_count": sum(row["status"] == "accepted" for row in audit),
                        "obstacle_count": len(output), "instances": audit}

    def recognize_floor_obstacles(input_dxf: Path | str, inventory_dir: Path | str, sheets_json: Path | str, output_dir: Path | str, *, config: ObstacleConfig=ObstacleConfig()) -> ObstacleRecognitionResult:
        ...
        layer_decisions = classify_layers_by_llm(rows + load_insert_semantic_rows(inventory_path), out_dir)
        doc = ezdxf.readfile(input_path)
        component_obstacles, component_audit = review_layer_components(doc, regions, layer_decisions, _s05_obstacles, config)
        rows = exclude_reviewed_component_rows(rows, component_audit)
        write_json(out_dir / 'mixed_layer_component_audit.json', component_audit)
        ...
        block_obstacles, block_audit = detect_candidate_block_obstacles(
            input_path, regions, layer_decisions, _s05_obstacles, config,
            doc=doc, component_audit=component_audit,
            existing_obstacles=area_obstacles + parallel_wall_obstacles + component_obstacles,
        )
        write_json(out_dir / 'block_parallel_obstacle_audit.json', block_audit)
        wall_processing_stats['block_parallel_rule_version'] = block_audit['rule_version']
        wall_processing_stats['block_parallel_obstacle_count'] = len(block_obstacles)
        wall_processing_stats['mixed_layer_component_obstacle_count'] = len(component_obstacles)
        wall_processing_stats['confirmed_door_count'] = component_audit['confirmed_door_count']
        wall_processing_stats['mixed_layer_component_audit'] = str(out_dir / 'mixed_layer_component_audit.json')
        obstacles = area_obstacles + parallel_wall_obstacles + block_obstacles + component_obstacles
        ...
        write_json(result_json, {'input_dxf': str(input_path), 'inventory_dir': str(inventory_path), 'sheets_json': str(sheets_path), 'output_dir': str(out_dir), 'strategy': 'LLM classifies trusted obstacle layers; closed/area geometry is clipped to inspection regions; open wall lines use projection-overlap cores plus topology-supported source-span, L/T junction, and end-cap completion.', 'layer_llm_decisions': str((out_dir / LAYER_LLM_FILE).resolve()), 'obstacle_csv': str(obstacle_csv.resolve()), 'obstacle_count': len(obstacles), 'obstacle_type_count': len(type_counts), 'region_count': len(regions), 'stage_counts': {'area_obstacles': len(area_obstacles), 'parallel_wall_line_bundle_obstacles': len(parallel_wall_obstacles), 'candidate_block_parallel_obstacles': len(block_obstacles), 'topology_completed_pair_obstacles': topology_completed_pair_count, 'topology_polygon_completion_obstacles': topology_polygon_completion_count, 'final_obstacles': len(obstacles)}, 'block_parallel_audit': str((out_dir / 'block_parallel_obstacle_audit.json').resolve()), 'block_parallel_rule_version': block_audit['rule_version'], 'wall_processing_stats': wall_processing_stats, 'topology_added_area': topology_added_area, 'type_counts': dict(type_counts.most_common()), 'region_counts': dict(region_counts.most_common()), 'per_region_geojsons': [str(path.resolve()) for path in per_region_geojsons], 'union_geojsons': [str(path.resolve()) for path in union_geojsons]})
        return ObstacleRecognitionResult(result_json=result_json, obstacle_csv=obstacle_csv, obstacle_count=len(obstacles), obstacle_type_count=len(type_counts), region_count=len(regions), per_region_geojsons=per_region_geojsons, union_geojsons=union_geojsons)
    return dict(locals())

_s05_obstacles = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.obstacle_recognition_door_mask_fixed',
