...
Candidate-block parallel bands are consumed from the same Stage 05 results:
candidate parent layer, at least two parallel pairs, and no contained arcs.
This renderer preserves that rule's audit provenance without reclassifying CAD.
...
# Never ship a provider credential in the reusable multi-drawing pipeline.
# Conditional building vision is enabled only when ARK_API_KEY is supplied by
...
class BuildingVectorValidationConfig:
    ...
    candidate_regions_constrain_vector_groups: bool = True
...
def _load_block_rule_audits(obstacle_paths: Iterable[Path]) -> list[dict[str, Any]]:
    """Attach upstream rule evidence; old runs without an audit remain readable."""
    records: list[dict[str, Any]] = []
    seen: set[Path] = set()
    for path in obstacle_paths:
        root = path.parent.parent if path.parent.name == "per_region_geojson" else path.parent
        for filename in ('block_parallel_obstacle_audit.json', 'mixed_layer_component_audit.json'):
            audit_path = root / filename
            if audit_path in seen or not audit_path.is_file():
                continue
            seen.add(audit_path)
            audit = _read_json(audit_path)
            records.append({
                "source": _fingerprint(audit_path),
                "summary": {key: value for key, value in audit.items()
                            if key not in {'instances', 'components', 'confirmed_doors', 'owned_components'}},
            })
    return records
    ...
    *,
    ...
    payload = {
        ...
        "sources": {
            ...
            "block_parallel_rule_audits": _load_block_rule_audits(obstacle_paths),
...
def _refine_detection_with_vector_mask(
    ...
    *,
    ...
    if config.candidate_regions_constrain_vector_groups:
...
def _remove_collinear_ring_vertices(points: Sequence[Sequence[float]]) -> list[tuple[float, float]]:
    """Remove cyclic seam vertices too; simplify() preserves a ring's start."""
    ...
    while len(ring) > 3:
        ...
        for index, point in enumerate(ring):
            ...
            tolerance = 1e-10 * max(1.0, math.hypot(ax, ay) * math.hypot(bx, by))
            if abs(ax * by - ay * bx) <= tolerance and ax * bx + ay * by >= 0:
                continue
            kept.append(point)
        if len(kept) < 3 or len(kept) == len(ring):
            break
        ring = kept
    return ring
...
def _snapped_outer_wall_completion(
    ...
    *,
    inward_search_ratio: float = 0.18,
    ...
    for polygon_index, polygon in enumerate(_polygonal_parts(structural_domain), start=1):
        # Unioned rectangles retain collinear vertices at former seams. Remove
        # those before intersecting adjacent shifted lines, otherwise a seam
        # vertex can remain unsnapped and introduce a diagonal exterior wall.
        cleaned_polygon = polygon.simplify(1e-7, preserve_topology=True)
        cad_points = list(cleaned_polygon.exterior.coords)[:-1]
        pixel_points = _remove_collinear_ring_vertices([
        ...
        max_depth = max(24, min(650, int(round(short_span * inward_search_ratio))))
