...
# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/context_features.py
# -----------------------------------------------------------------------------
def _build_s08_context():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/semantic/context_features.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic.context_features'
...
_s08_context = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.semantic.context_features',
    _build_s08_context(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/euclidean_closure.py
# -----------------------------------------------------------------------------


# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/route_optimizer.py
# -----------------------------------------------------------------------------
def _build_s08_optimizer():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/semantic/route_optimizer.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic.route_optimizer'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic'
...
_s08_optimizer = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.semantic.route_optimizer',
    _build_s08_optimizer(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/target_selector.py
# -----------------------------------------------------------------------------
def _build_s08_selector():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/semantic/target_selector.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic.target_selector'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic'
    ...
    DEFAULT_CONSTRAINTS_PATH = (
        Path(globals()["__file__"]).resolve().parents[2]
        / 'configs'
        / 'semantic_inspection_constraints.json'
    )
    ...
    def _legacy_helpers() -> tuple[Any, Any, Any, Any]:
        ...
        try:
            ...
            try:
                from multi_drawing_pipeline.stages.fire_route_core.inspection_route_planning import annotated_object_name, annotation_sort_key, candidates_for_rule_class, standard_class_name
                return (annotated_object_name, annotation_sort_key, candidates_for_rule_class, standard_class_name)
            except Exception:
                pass
        try:
            ...
            def rank(item: dict[str, Any]) -> tuple[int, int, tuple[Any, ...]]:
                ...
                try:
                    try:
                        ...
                        from multi_drawing_pipeline.stages.fire_route_core.inspection_route_planning import original_name_rule_class
...
_s08_selector = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.semantic.target_selector',
    _build_s08_selector(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/graph_builder.py
# -----------------------------------------------------------------------------


# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/pipeline.py
# -----------------------------------------------------------------------------
def _build_s08_semantic_pipeline():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/semantic/pipeline.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic.pipeline'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic'
    ...
    from multi_drawing_pipeline.stages.fire_route_core.semantic.context_features import ContextFeatureConfig, compute_context_features, write_context_features
    from multi_drawing_pipeline.stages.fire_route_core.semantic.route_optimizer import optimize_floor_route
    from multi_drawing_pipeline.stages.fire_route_core.semantic.target_selector import build_target_candidates, load_constraints
    SCHEMA_VERSION = 1
...
_s08_semantic_pipeline = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.semantic.pipeline',
    _build_s08_semantic_pipeline(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/pretraining_dataset.py
# -----------------------------------------------------------------------------
def _build_s08_dataset():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/semantic/pretraining_dataset.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic.pretraining_dataset'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic'
    ...
    def _build_raw_floor(source_run_id: str, floor_row: Mapping[str, Any], run: Mapping[str, Any], constraints: Mapping[str, Any], *, near_radius_factor: float=0.15, near_max_neighbors: int=32) -> _RawFloorGraph:
        ...
        for edge in physical_edges:
            ...
            features: dict[str, Any] = {'distance': edge.get('routing_cost', edge.get('length'))}
...
_s08_dataset = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.semantic.pretraining_dataset',
    _build_s08_dataset(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/R-GNN/edge_gated_rgcn_pretraining.py
# -----------------------------------------------------------------------------
def _build_s08_rgcn_model():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/R-GNN/edge_gated_rgcn_pretraining.py'
    )
    __name__ = 'edge_gated_rgcn_pretraining'
    __package__ = ''
    """Pure-PyTorch edge-gated relational pretraining for semantic navigation.

    This model/training module lives in ``multi_drawing_pipeline.stages.fire_route_core/R-GNN``; graph
    construction and tensorization remain in ``multi_drawing_pipeline.stages.fire_route_core/semantic``.
    """
    import json
    import math
    from dataclasses import asdict, dataclass
...
# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/R-GNN/pseudo_label_finetuning.py
# -----------------------------------------------------------------------------
def _build_s08_route_heads():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/R-GNN/pseudo_label_finetuning.py'
    )
...
# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/R-GNN/export_route_recommendations.py
# -----------------------------------------------------------------------------
def _build_s08_route_export():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/R-GNN/export_route_recommendations.py'
    )
...
# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/R-GNN/export_full_floor_recommendations.py
# -----------------------------------------------------------------------------
def _build_s08_full_export():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/R-GNN/export_full_floor_recommendations.py'
    )
    ...
    from multi_drawing_pipeline.stages.fire_route_core.semantic.pretraining_dataset import _build_raw_floor, _tensorize_graphs
    from multi_drawing_pipeline.stages.fire_route_core.semantic.target_selector import load_constraints
    from multi_drawing_pipeline.stages.fire_route_core.dual_graph.physical_access import augment_virtual_target_access_nodes
...
# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/dual_graph/runtime_pipeline.py
# -----------------------------------------------------------------------------
def _build_s08_runtime():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/dual_graph/runtime_pipeline.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.dual_graph.runtime_pipeline'
    ...
    from multi_drawing_pipeline.stages.fire_route_core.dual_graph.physical_access import augment_virtual_target_access_nodes
    from multi_drawing_pipeline.stages.fire_route_core.semantic.context_features import ContextFeatureConfig, compute_context_features, write_context_features
    from multi_drawing_pipeline.stages.fire_route_core.semantic.physical_metric_closure import ClosureBuildConfig, PhysicalMetricClosure, build_physical_metric_closure
    from multi_drawing_pipeline.stages.fire_route_core.semantic.pipeline import DEFAULT_EMPTY_GAP_RATIO, DEFAULT_HARD_GAP_RATIO, _decorate_candidates, _flatten_candidates, _insertion_counterfactuals, _route_with_values
    from multi_drawing_pipeline.stages.fire_route_core.semantic.target_selector import build_target_candidates, load_constraints
    ...
    def build_runtime_candidates_with_context(run_dir: Path | str, refined_graph_path: Path | str, output_dir: Path | str, *, constraints_path: Path | str | None=None, near_radius_factor: float=0.15) -> dict[str, Any]:
        ...
        output = Path(output_dir).resolve()
        output.mkdir(parents=True, exist_ok=True)
        graph_path = _require_file(refined_graph_path, '物理导航图')
...
_s08_runtime = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.dual_graph.runtime_pipeline',
    _build_s08_runtime(),
    aliases=(),
)
