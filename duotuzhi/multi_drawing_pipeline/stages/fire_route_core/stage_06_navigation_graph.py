...
from multi_drawing_pipeline.stages.fire_route_core import stage_04_inspection_objects as _stage04
from multi_drawing_pipeline.stages.fire_route_core import stage_05_obstacles as _stage05

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/navigation_input_adapter.py
# -----------------------------------------------------------------------------
def _build_s06_inputs():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/navigation_input_adapter.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.navigation_input_adapter'
    ...
    from shapely.geometry import GeometryCollection, Point, Polygon, box, mapping, shape
    ...
    def _floor_domains(sheets_json: Path) -> tuple[dict[str, Any], dict[str, str], dict[str, list[dict[str, Any]]]]:
        ...
        for region in load_floor_regions(sheets_json):
            ...
            members[floor_id].append({
                'source_floor_id': str(region.get('source_floor_id') or floor_id),
                'building_scope_id': str(region.get('building_scope_id') or floor_id),
                'building_id': str(region.get('building_id') or ''),
                'building_name': str(region.get('building_name') or ''),
                'corridor_connection_group': str(region.get('corridor_connection_group') or ''),
                'corridor_connection_verified': bool(region.get('corridor_connection_verified')),
                'corridor_connection_verification': str(region.get('corridor_connection_verification') or ''),
        ...
        return (domains, names, dict(members))
    ...
    def _load_obstacle_unions(paths: list[Path], floor_domains: dict[str, Any]) -> tuple[dict[str, Any], int, int, bool, set[str], dict[str, dict[str, float]]]:
        ...
        for path in paths:
            ...
            for index, feature in enumerate(features, start=1):
                ...
                scope_floor_ids = [floor_id] if floor_id in floor_domains else [
                    scope_id for scope_id in floor_domains if scope_id.startswith(floor_id + '__')
                ]
                ...
                if not scope_floor_ids:
                    feature_count += 1
                    continue
                covered_floors.update(scope_floor_ids)
                if not feature.get('geometry'):
                    ...
                    for scope_id in scope_floor_ids:
                        if grouped.get(scope_id):
                            raise ValueError(f'Floor/building scope {scope_id} has both non-empty obstacle geometry and obstacle_count=0')
                        explicit_empty_floors.add(scope_id)
                    feature_count += 1
                    continue
                conflicting_scopes = [scope_id for scope_id in scope_floor_ids if scope_id in explicit_empty_floors]
                if conflicting_scopes:
                    raise ValueError(f"Floor/building scope {conflicting_scopes[0]} has both obstacle_count=0 and non-empty obstacle geometry")
                try:
                ...
                for scope_id in scope_floor_ids:
                    if geometry.intersects(floor_domains[scope_id]):
                        grouped[scope_id].append(geometry)
                feature_count += 1
        result: dict[str, Any] = {}
    ...
    def _merge_vector_verified_corridor_scopes(
        floor_domains: dict[str, Any],
        floor_names: dict[str, str],
        scope_members: dict[str, list[dict[str, Any]]],
        obstacles_by_floor: dict[str, Any],
        covered_obstacle_floors: set[str],
        clipping_stats: dict[str, dict[str, float]],
    ) -> tuple[
        dict[str, Any],
        dict[str, str],
        ...
        """Merge same-floor building scopes only across an open shared boundary."""

        scope_source_floor: dict[str, str] = {}
        for scope_id in floor_domains:
            source_ids = {
                str(member.get('source_floor_id') or '').strip()
                for member in scope_members.get(scope_id, [])
                if str(member.get('source_floor_id') or '').strip()
            }
            scope_source_floor[scope_id] = (
                next(iter(source_ids)) if len(source_ids) == 1 else scope_id.split('__', 1)[0]
            )

        free_by_scope: dict[str, Any] = {}
        for scope_id, domain in floor_domains.items():
            obstacle = obstacles_by_floor.get(scope_id, GeometryCollection())
            free_by_scope[scope_id] = _strict_polygonal(
                domain.difference(obstacle),
                f'corridor validation free area for {scope_id}',
                allow_empty=True,
            )

        parent = {scope_id: scope_id for scope_id in floor_domains}

        def find(scope_id: str) -> str:
            while parent[scope_id] != scope_id:
                parent[scope_id] = parent[parent[scope_id]]
        ...
        scope_ids = sorted(floor_domains)
        for left_index, left_id in enumerate(scope_ids):
            left_free = free_by_scope[left_id]
            if left_free.is_empty:
                continue
            left_buildings = {
                str(member.get('building_id') or '').strip()
                for member in scope_members.get(left_id, [])
                if str(member.get('building_id') or '').strip()
            }
            if not left_buildings:
                continue
            left_trusted_connection_groups = {
                str(member.get('corridor_connection_group') or '').strip()
                for member in scope_members.get(left_id, [])
                if member.get('corridor_connection_verified')
                and str(member.get('corridor_connection_verification') or '').strip().lower()
                in {'manual_verified', 'cad_portal_verified', 'door_portal_verified'}
                and str(member.get('corridor_connection_group') or '').strip()
            }
            for right_id in scope_ids[left_index + 1:]:
                if scope_source_floor[left_id] != scope_source_floor[right_id]:
                    continue
                right_buildings = {
                    str(member.get('building_id') or '').strip()
                    for member in scope_members.get(right_id, [])
                    if str(member.get('building_id') or '').strip()
                }
                if not right_buildings or left_buildings.intersection(right_buildings):
                    continue
                right_trusted_connection_groups = {
                    str(member.get('corridor_connection_group') or '').strip()
                    for member in scope_members.get(right_id, [])
                    if member.get('corridor_connection_verified')
                    and str(member.get('corridor_connection_verification') or '').strip().lower()
                ...
                trusted_groups = left_trusted_connection_groups.intersection(
                    right_trusted_connection_groups
                )
                if not trusted_groups:
                    continue
                right_free = free_by_scope[right_id]
                if right_free.is_empty:
                    continue
                open_shared = left_free.boundary.intersection(right_free.boundary)
                if open_shared.is_empty:
                    continue
                left_bounds = floor_domains[left_id].bounds
                right_bounds = floor_domains[right_id].bounds
                ...
                minimum_opening = max(1.0, float(reference_short_side) * 0.01)
                open_length = float(open_shared.length)
                if open_length < minimum_opening:
                    continue
                combined_obstacles = unary_union([
                    obstacles_by_floor.get(left_id, GeometryCollection()),
                    obstacles_by_floor.get(right_id, GeometryCollection()),
                ])
                corridor = _strict_polygonal(
                    open_shared.buffer(max(1.0, minimum_opening * 0.25), cap_style=2)
                    .intersection(floor_domains[left_id].union(floor_domains[right_id]))
                    .difference(combined_obstacles),
                    f'verified corridor between {left_id} and {right_id}',
                    allow_empty=True,
                )
                ...
                raw_connections.append({
                    'left_scope_id': left_id,
                    'right_scope_id': right_id,
                    'source_floor_id': scope_source_floor[left_id],
                    'left_building_ids': sorted(left_buildings),
                    'right_building_ids': sorted(right_buildings),
                    'open_boundary_length': open_length,
                    'minimum_opening': minimum_opening,
                    'trusted_connection_group': min(trusted_groups),
                    'validation_method': 'trusted_connection_group_plus_shared_free_boundary_after_obstacle_subtraction',
                    'corridor_geometry': mapping(corridor),
                })

        components: dict[str, list[str]] = defaultdict(list)
        for scope_id in scope_ids:
            components[find(scope_id)].append(scope_id)
        connection_index_by_root: dict[str, int] = {}
        per_floor_counter: dict[str, int] = defaultdict(int)
        for root, members in sorted(components.items(), key=lambda item: min(item[1])):
            if len(members) <= 1:
                continue
            source_floor_id = scope_source_floor[members[0]]
            per_floor_counter[source_floor_id] += 1
            connection_index_by_root[root] = per_floor_counter[source_floor_id]

        scope_remap: dict[str, str] = {}
        for root, members in components.items():
            if len(members) <= 1:
                scope_remap[members[0]] = members[0]
                continue
            source_floor_id = scope_source_floor[members[0]]
            group_index = connection_index_by_root[root]
            merged_scope_id = f'{source_floor_id}__CONN_AUTO_C{group_index:03d}'
            for scope_id in members:
                scope_remap[scope_id] = merged_scope_id

        merged_domains: dict[str, list[Any]] = defaultdict(list)
        merged_obstacles: dict[str, list[Any]] = defaultdict(list)
        merged_names: dict[str, str] = {}
        merged_members: dict[str, list[dict[str, Any]]] = defaultdict(list)
        merged_covered: set[str] = set()
        merged_clipping: dict[str, dict[str, float]] = defaultdict(
            lambda: {'source_area': 0.0, 'clipped_area': 0.0, 'outside_domain_area': 0.0}
        )
        for old_scope_id, domain in floor_domains.items():
            new_scope_id = scope_remap[old_scope_id]
            merged_domains[new_scope_id].append(domain)
            obstacle = obstacles_by_floor.get(old_scope_id)
            if obstacle is not None and not obstacle.is_empty:
                merged_obstacles[new_scope_id].append(obstacle)
            merged_names.setdefault(new_scope_id, floor_names.get(old_scope_id, new_scope_id))
            merged_members[new_scope_id].extend(scope_members.get(old_scope_id, []))
            if old_scope_id in covered_obstacle_floors:
                merged_covered.add(new_scope_id)
            old_stats = clipping_stats.get(old_scope_id, {})
            for key in ('source_area', 'clipped_area', 'outside_domain_area'):
                merged_clipping[new_scope_id][key] += float(old_stats.get(key, 0.0))

        normalized_domains = {
            scope_id: _strict_polygonal(unary_union(geometries), f'merged corridor domain {scope_id}')
            for scope_id, geometries in merged_domains.items()
        }
        normalized_obstacles = {
            scope_id: _strict_polygonal(
                unary_union(geometries).intersection(normalized_domains[scope_id]),
                f'merged corridor obstacle {scope_id}',
            )
            for scope_id, geometries in merged_obstacles.items()
            if geometries
        }

        connections: list[dict[str, Any]] = []
        for index, connection in enumerate(raw_connections, start=1):
            merged_scope_id = scope_remap[connection['left_scope_id']]
            connections.append({
                'corridor_id': f'NAV_CORRIDOR_{index:03d}',
                'corridor_connection_group': merged_scope_id.split('__CONN_', 1)[-1],
                'building_scope_id': merged_scope_id,
                **connection,
            })
        return (
            normalized_domains,
    ...
    def _target_scope(
        ...
        """Resolve legacy floor-only targets into a floor/building domain."""
        point = Point((bbox[0] + bbox[2]) / 2.0, (bbox[1] + bbox[3]) / 2.0)
        declared_scope = str(annotation.get('building_scope_id') or annotation.get('floor_id') or '').strip()
        if declared_scope in floor_domains and floor_domains[declared_scope].covers(point):
            return (declared_scope, point)
        source_floor_id = str(annotation.get('source_floor_id') or annotation.get('floor_id') or '').strip()
        candidate_ids = [
            floor_id for floor_id in floor_domains
            if floor_id == source_floor_id or floor_id.startswith(source_floor_id + '__')
        ]
        containing = [floor_id for floor_id in candidate_ids if floor_domains[floor_id].covers(point)]
        if containing:
            return (min(containing, key=lambda floor_id: float(floor_domains[floor_id].area)), point)
        target_box = box(*bbox)
        target_area = max(float(target_box.area), 1.0)
        ranked = [
            (float(target_box.intersection(floor_domains[floor_id]).area) / target_area, floor_id)
            for floor_id in candidate_ids
        ]
        if not ranked:
            return None
        ratio, floor_id = max(ranked, key=lambda item: item[0])
        return (floor_id, point) if ratio >= 0.5 else None

    def _target_features(inspection_output_dir: Path, floor_domains: dict[str, Any], scope_members: dict[str, list[dict[str, Any]]] | None=None) -> tuple[list[dict[str, Any]], int, int, int]:
        ...
        invalid_source_count = sum((1 for annotation in source_annotations if _valid_bbox(annotation.get('bbox')) is None))
        if invalid_source_count:
            raise RuntimeError(f'Collected {invalid_source_count} inspection targets with invalid bbox')
        ...
        for annotation in annotations:
            ...
            if bbox is None:
                skipped += 1
                continue
            resolved = _target_scope(annotation, bbox, floor_domains)
            if resolved is None:
                skipped += 1
                continue
            floor_id, point = resolved
            source_floor_id = str(annotation.get('source_floor_id') or annotation.get('floor_id') or floor_id).strip()
            building_id = str(annotation.get('building_id') or '').strip()
            building_name = str(annotation.get('building_name') or '').strip()
            for member in (scope_members or {}).get(floor_id, []):
                if member['polygon'].covers(point):
                    building_id = building_id or str(member.get('building_id') or '')
                    building_name = building_name or str(member.get('building_name') or building_id)
                    break
            if not building_id and floor_id.startswith(source_floor_id + '__'):
                suffix = floor_id[len(source_floor_id) + 2:]
                if not suffix.startswith('CONN_'):
                    building_id = suffix
            ...
            features[-1]['properties'].update({
                ...
                'corridor_connection_group': (
                    floor_id.split('__CONN_', 1)[1]
                    if '__CONN_' in floor_id
                    else ''
                ),
            })
    ...
    def prepare_navigation_inputs(sheets_json: Path | str, obstacle_union_geojsons: Iterable[Path | str], inspection_output_dir: Path | str, output_dir: Path | str, write_debug_dxf: bool=False, *, expected_obstacle_count: int | None=None, expected_target_count: int | None=None) -> NavigationInputResult:
        ...
        floor_domains, floor_names, floor_scope_members = _floor_domains(sheets_path)
        ...
        (
            ...
            floor_scope_members,
            obstacles_by_floor,
            covered_obstacle_floors,
            clipping_stats,
            corridor_connections,
            corridor_scope_remap,
        ) = _merge_vector_verified_corridor_scopes(
            floor_domains,
            floor_names,
            floor_scope_members,
            obstacles_by_floor,
        ...
        for floor_id, domain in floor_domains.items():
            member_rows = floor_scope_members.get(floor_id, [])
            source_floor_ids = sorted({str(item.get('source_floor_id') or floor_id) for item in member_rows})
            building_ids = sorted({str(item.get('building_id') or '') for item in member_rows if item.get('building_id')})
            scope_properties = {
                'source_floor_id': source_floor_ids[0] if len(source_floor_ids) == 1 else '',
                'building_scope_id': floor_id,
                'building_id': building_ids[0] if len(building_ids) == 1 else '',
            ...
            if not obstacle.is_empty:
                obstacle_features.append({'type': 'Feature', 'properties': {'floor_id': floor_id, 'floor_name': floor_names[floor_id], 'kind': 'obstacle_union', **scope_properties}, 'geometry': mapping(obstacle)})
            try:
            ...
            if free_area.is_empty:
                ...
                continue
            free_features.append({'type': 'Feature', 'properties': {'floor_id': floor_id, 'floor_name': floor_names[floor_id], 'kind': 'free_area', **scope_properties}, 'geometry': mapping(free_area)})
        ...
        targets, skipped_target_count, source_target_count, deduplicated_source_target_count = _target_features(inspection_dir, floor_domains, floor_scope_members)
...
_s06_inputs = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.navigation_input_adapter',
    _build_s06_inputs(),
    aliases=('navigation_input_adapter',),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/portal_area_graph.py
# -----------------------------------------------------------------------------
def _build_s06_area_graph():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/portal_area_graph.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.portal_area_graph'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core'
    ...
    def build_portal_area_graph(free_area_path: Path, obstacle_path: Path, output_dir: Path, source_dxf: Path | None=None, config: BuildConfig | None=None) -> dict[str, Any]:
        ...
        outputs = {'portal_geojson': output_dir / 'portal_candidates.geojson', 'portal_csv': output_dir / 'portal_candidates.csv', 'regions_geojson': output_dir / 'area_graph_regions.geojson', 'area_graph_json': output_dir / 'area_graph.json', 'summary_json': output_dir / 'area_graph_summary.json'}
...
_s06_area_graph = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.portal_area_graph',
    _build_s06_area_graph(),
    aliases=('portal_area_graph',),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/area_graph_navigation.py
# -----------------------------------------------------------------------------
def _build_s06_navigation():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/area_graph_navigation.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.area_graph_navigation'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core'
    ...
    def build_navigation_graph(run_dir: Path, output_dir: Path | None=None, source_dxf: Path | None=None, config: NavConfig | None=None) -> dict[str, Any]:
...
_s06_navigation = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.area_graph_navigation',
    _build_s06_navigation(),
    aliases=('area_graph_navigation',),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/navigation_pipeline.py
# -----------------------------------------------------------------------------
def _build_s06_pipeline():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/navigation_pipeline.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.navigation_pipeline'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core'
    import hashlib
    from dataclasses import dataclass
    from pathlib import Path
...
_s06_pipeline = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.navigation_pipeline',
    _build_s06_pipeline(),
...
def run_stage(
    *,
    ...
    from multi_drawing_pipeline.stages.fire_route_core.navigation_obstacles import approved_manifest
    approved = approved_manifest(run_dir)
    if approved:
        # Replace legacy closed envelopes; do not seal the reviewed door cuts.
        obstacle_union_geojsons = [Path(approved["geojson_path"])]
        expected_obstacle_count = int(approved["obstacle_count"])
