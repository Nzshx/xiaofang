...
from fire_inspection_system.business_image_outputs import generate_business_image_outputs
from fire_inspection_system.route_beautification import run_main_route_beautification
...
DISCIPLINE_SCOPE_CONFIG = (
    MODULE_DIR
    / "duotuzhi"
    / "multi_drawing_pipeline"
    / "configs"
    / "discipline_object_scope.json"
)
...
def _choose_cad_files(title: str, *, multiple: bool) -> list[Path]:
...
def _ask_yes_no(prompt: str) -> bool:
    while True:
        try:
            ...
            raise RuntimeError("当前控制台不能读取Y/N，请使用命令行参数显式指定图纸") from exc
        if answer in {"Y", "YES"}:
            return True
        if answer in {"N", "NO"}:
            return False
...
def _select_optional_professional_drawings(label: str) -> list[Path]:
    if not _ask_yes_no(f"是否选择{label}专业图纸？"):
        return []
    selected = _choose_cad_files(
        f"选择{label}专业 DWG/DXF（可多选）",
        multiple=True,
    )
    if not selected:
        print(f"未选择{label}专业图纸，按 N 处理。")
    return selected


def _should_open_input_wizard(args: argparse.Namespace) -> bool:
    return not bool(
        args.input
        or args.building
        or args.water
        ...
        or args.interactive_project
        or args.manual_files
        or args.scan_only
        or args.path_planning_only
        or args.start_stage != 1
    )


def _run_input_wizard(args: argparse.Namespace) -> None:
    print("\n[图纸选择] 首先选择唯一建筑物底图")
    building = _choose_cad_files("选择唯一建筑物底图 DWG/DXF", multiple=False)
    if not building:
        raise RuntimeError("未选择建筑物底图，程序已取消。")
    water = _select_optional_professional_drawings("给排水")
    electrical = _select_optional_professional_drawings("电气")
    hvac = _select_optional_professional_drawings("暖通")
    print(f"  建筑: {building[0]}")
    print(f"  给排水: {len(water)} 张")
    print(f"  电气: {len(electrical)} 张")
    print(f"  暖通: {len(hvac)} 张")
    if water or electrical or hvac:
        args.multi_drawing = True
        args.input = ""
        args.building = [str(building[0])]
        args.water = [str(path) for path in water]
        args.electrical = [str(path) for path in electrical]
        args.hvac = [str(path) for path in hvac]
        print("[执行模式] 建筑底图 + 专业图纸融合流程")
    else:
        args.multi_drawing = False
        args.input = str(building[0])
        args.building = []
        print("[执行模式] 仅建筑底图，使用 fire_inspection_system/stages 十一阶段流程")


def _normalise_building_only_cli(args: argparse.Namespace) -> None:
    """Allow --building alone to select the original single-building pipeline."""
    if _uses_multi_drawing_pipeline(args) or not args.building:
        return
    if len(args.building) != 1:
        raise ValueError("仅建筑模式必须且只能提供一张 --building 图纸")
    building = str(Path(args.building[0]).expanduser().resolve())
    if args.input and Path(_strip_quotes(args.input)).expanduser().resolve() != Path(building):
        raise ValueError("--input 与 --building 指向了不同建筑图纸")
    args.input = building
    args.building = []


def _building_allowed_classes() -> frozenset[str]:
    if not DISCIPLINE_SCOPE_CONFIG.is_file():
        raise FileNotFoundError(f"专业对象范围配置不存在: {DISCIPLINE_SCOPE_CONFIG}")
    payload = json.loads(DISCIPLINE_SCOPE_CONFIG.read_text(encoding="utf-8"))
    values = (payload.get("building") or {}).get("allowed_classes") or []
    allowed = frozenset(str(value).strip() for value in values if str(value).strip())
    if not allowed:
        raise ValueError("建筑专业对象白名单为空")
    return allowed
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--multi-drawing",
        action="store_true",
        help="启用建筑+可选水/电/暖通多图纸融合流程",
    )
    parser.add_argument("--project-root", default="", help="自动扫描并分类图纸的项目目录")
    parser.add_argument(
        "--building", action="append", default=[],
        help="建筑DWG/DXF；单独使用时进入原十一阶段，与专业图参数组合时进入融合流程",
    )
    parser.add_argument("--water", action="append", default=[], help="水专业DWG/DXF，可重复")
    parser.add_argument("--electrical", action="append", default=[], help="电专业DWG/DXF，可重复")
    parser.add_argument("--hvac", action="append", default=[], help="暖通专业DWG/DXF，可重复")
    parser.add_argument("--interactive-project", action="store_true", help="弹窗选择多图纸项目目录")
    parser.add_argument("--manual-files", action="store_true", help="弹窗按专业选择图纸")
    parser.add_argument("--scan-only", action="store_true", help="只扫描项目并显示专业分类")
    parser.add_argument(
        "--start-stage", type=int, choices=range(1, 8), default=1,
        help="多图纸流程从指定阶段续跑",
    )
    parser.add_argument(
    ...
    parser.add_argument("--manual-anchors", default="", help="专业图人工配准锚点JSON")
    parser.add_argument(
        "--rebuild-route-targets", action="store_true",
        help="根据已有识别结果重建全专业Object Set路径目标",
    )
    parser.add_argument(
        "--artifact-root", default="",
        help="多图纸SBM/Object Set/Environment/Route内容寻址版本库",
    )
    parser.add_argument(
        "--skip-obstacle-annotation",
        action="store_true",
        help="跳过建筑障碍物逐对象标注DXF写出（大图可超过100MB；仅省略人审展示文件）",
    )
    parser.add_argument(
        "--path-planning-only",
        action="store_true",
...
def _uses_multi_drawing_pipeline(args: argparse.Namespace) -> bool:
    return bool(
        args.multi_drawing
        or args.project_root
        or args.water
...
def _run_multi_drawing_pipeline(args: argparse.Namespace) -> dict[str, object]:
    from fire_inspection_system.duotuzhi.run_pipeline import run_pipeline

    if args.path_planning_only:
        raise ValueError("多图纸模式请使用 --start-stage 续跑，不能使用 --path-planning-only")
    building = [Path(value).expanduser() for value in args.building]
    if not building and args.input:
        building = [Path(_strip_quotes(args.input)).expanduser()]
    options = argparse.Namespace(
        project_root=Path(args.project_root).expanduser() if args.project_root else None,
        building=building,
        water=[Path(value).expanduser() for value in args.water],
        electrical=[Path(value).expanduser() for value in args.electrical],
        hvac=[Path(value).expanduser() for value in args.hvac],
        interactive=bool(args.interactive_project),
        manual_files=bool(args.manual_files),
        ...
        registration_policy=str(args.registration_policy),
        manual_anchors=Path(args.manual_anchors).expanduser() if args.manual_anchors else None,
        rebuild_route_targets=bool(args.rebuild_route_targets),
        skip_route_planning=bool(args.no_path_planning),
        no_path_dxf=bool(args.no_path_dxf),
        path_device=str(args.path_device),
        path_transition_top_k=int(args.path_transition_top_k),
        path_dmax_ratio=float(args.path_dmax_ratio),
        area_graph_pixel_size=float(args.area_graph_pixel_size or 240.0),
        rgcn_dataset_manifest=Path(args.rgcn_dataset_manifest).expanduser(),
        rgcn_checkpoint=Path(args.rgcn_checkpoint).expanduser(),
        route_head_checkpoint=Path(args.route_head_checkpoint).expanduser(),
        no_llm=bool(args.no_llm),
        skip_obstacle_annotation=bool(getattr(args, "skip_obstacle_annotation", False)),
        artifact_root=(
            Path(args.artifact_root).expanduser()
            if args.artifact_root
            else MODULE_DIR / "duotuzhi" / "artifacts"
        ),
    )
...
def _resolve_resume_input(
    ...
    obstacle_result_path = run_dir / "obstacles" / "floor_obstacle_recognition_result.json"
    obstacle_result = (
        json.loads(obstacle_result_path.read_text(encoding="utf-8"))
        if obstacle_result_path.is_file()
        else {}
    )
    raw_input = (
        _strip_quotes(args.input)
        if args.input
        else str(summary.get("input_dxf") or obstacle_result.get("input_dxf") or "")
    )
    if not raw_input:
        raise ValueError("续跑目录无法恢复 input_dxf，请显式传入 --input")
    input_dxf = Path(raw_input).expanduser().resolve()
    if not summary:
        summary = {
            "input_cad": str(input_dxf),
            "input_dxf": str(input_dxf),
            "run_dir": str(run_dir.resolve()),
            "resume_recovered_without_previous_pipeline_summary": True,
            "cad_preprocess": {
                "sheets_json": str(obstacle_result.get("sheets_json") or ""),
            },
            "vector_inventory": {
                "inventory_dir": str(obstacle_result.get("inventory_dir") or ""),
            },
            "obstacle_recognition": {
                "result_json": str(obstacle_result_path.resolve()),
                "union_geojsons": list(obstacle_result.get("union_geojsons") or []),
                "obstacle_count": int(obstacle_result.get("obstacle_count") or 0),
            },
        }
    return input_dxf, summary
...
def _run_path_stages(
    ...
    path_result, summary_path = stage_10_route_outputs.run_stage(
        ...
        semantic=semantic,
        planning=planning,
        # The final DXF is deliberately delayed until the display geometry has
        # been beautified and re-certified.  Stage 10's original GeoJSON/JSON
        # remains unchanged as the safety-audit source of truth.
        write_dxf=False,
    )
    ...
    print("\n[路线美化] 安全路线视线直连、逐段复核并生成最终 DXF")
    beautification = run_main_route_beautification(
        run_dir,
        input_dxf,
    ...
    outputs.update(
        {
            "original_forwarding_route_geojson": beauty_outputs.get(
                "original_route_geojson", ""
            ),
            "display_route_geojson": beauty_outputs.get(
                "beautified_route_geojson", ""
            ),
            "route_beautification_audit": beauty_outputs.get("audit_json", ""),
            "annotated_route_dxf": beauty_outputs.get("annotated_route_dxf", ""),
        }
    )
    path_result["route_beautification"] = beautification
    path_result.setdefault("route_safety_policy", {})[
        "display_route_recertified_after_beautification"
    ] = True
    dual = path_result.get("dual_graph") or {}
    dual_outputs = dual.setdefault("outputs", {})
    dual_outputs["original_forwarding_route_geojson"] = outputs[
        "original_forwarding_route_geojson"
    ]
    dual_outputs["display_route_geojson"] = outputs["display_route_geojson"]
    dual_outputs["route_beautification_audit"] = outputs[
        "route_beautification_audit"
    ]
    dual_outputs["annotated_route_dxf"] = outputs["annotated_route_dxf"]
    dual.setdefault("config", {})["write_dxf"] = not args.no_path_dxf
    dual["config"]["route_beautification_enabled"] = True
    dual_summary_value = outputs.get("dual_graph_summary")
    if dual_summary_value:
        Path(dual_summary_value).write_text(
            json.dumps(dual, ensure_ascii=False, indent=2, allow_nan=False),
            encoding="utf-8",
        )
    path_summary_path = Path(
        path_result.get("summary_path")
        or run_dir / "path_planning" / "path_planning_summary.json"
    )
    path_result["summary_path"] = str(path_summary_path.resolve())
    path_summary_path.write_text(
        json.dumps(path_result, ensure_ascii=False, indent=2, allow_nan=False),
        encoding="utf-8",
    )
    pipeline_summary["path_planning"] = path_result
    pipeline_summary["route_beautification"] = beautification
    summary_path.write_text(
        json.dumps(pipeline_summary, ensure_ascii=False, indent=2, allow_nan=False),
        encoding="utf-8",
    )
    print(f"  original_route: {outputs['original_forwarding_route_geojson']}")
    print(f"  display_route: {outputs['display_route_geojson']}")
    print(f"  audit: {outputs['route_beautification_audit']}")
    if outputs["annotated_route_dxf"]:
        print(f"  final_dxf: {outputs['annotated_route_dxf']}")
...
def main() -> None:
    args = parse_args()
    if _should_open_input_wizard(args):
        _run_input_wizard(args)
    _normalise_building_only_cli(args)
    _apply_llm_api_config(args)
    if _uses_multi_drawing_pipeline(args):
        _run_multi_drawing_pipeline(args)
        return
    if args.path_planning_only:
        ...
        print("\n[业务图片] 生成障碍物图、路线点位图和无路线视觉输入图")
        business_images = generate_business_image_outputs(run_dir)
        summary["business_image_outputs"] = business_images
        summary_path.write_text(
    ...
    recognition_result = stage_04_inspection_objects.run_stage(
        ...
        cad.run_dir,
        no_llm=args.no_llm,
        allowed_classes=_building_allowed_classes(),
    )
    ...
    if args.no_path_planning:
        ...
        print(f"  report_index: {acceptance_result.index_markdown}")
        print("\n[业务图片] 生成障碍物图、路线点位图和无路线视觉输入图")
        business_images = generate_business_image_outputs(cad.run_dir)
        summary["business_image_outputs"] = business_images
        summary_path.write_text(
            json.dumps(summary, ensure_ascii=False, indent=2, allow_nan=False),
            encoding="utf-8",
