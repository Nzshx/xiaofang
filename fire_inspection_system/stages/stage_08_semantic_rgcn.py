...
def _build_s08_optimizer():
    ...
    def optimize_floor_route(target_candidates: Mapping[str, Any] | Sequence[Mapping[str, Any]], distance_matrix: Mapping[str, Any], floor_id: str | None=None, *, max_exact_targets: int=16, max_selection_combinations: int=50000, distance_tolerance: float=0.0, target_values: Mapping[str, float] | None=None, soft_gap_distance: float | None=None, gap_penalty_weight: float=0.0, max_segment_distance: float | None=None) -> dict[str, Any]:
        ...
        for count in option_counts:
            ...
            if total_combinations > max_selection_combinations:
                break
        exact_mode = bool(len(fixed) <= max_exact_targets and total_combinations <= max_selection_combinations and all((count > 0 for count in option_counts)))
...
def _build_s08_semantic_pipeline():
    ...
    def _route_with_values(bundle: Mapping[str, Any], distance_matrix: Mapping[str, Any], context: Mapping[str, Any], *, delta: float, empty_gap_by_floor: Mapping[str, float], hard_gap_by_floor: Mapping[str, float]) -> dict[str, Any]:
        ...
        for floor_id, floor in sorted((bundle.get('floors', {}) or {}).items()):
            floors[floor_id] = optimize_floor_route(floor, distance_matrix, floor_id, distance_tolerance=delta, target_values=values, soft_gap_distance=empty_gap_by_floor.get(floor_id), max_segment_distance=hard_gap_by_floor.get(floor_id))
        ...
        return {'schema_version': SCHEMA_VERSION, 'route_type': 'same_floor_open_constrained_route', 'distance_metric': 'euclidean_target_centres_approximation', 'physical_path_available': False, 'delta': delta, 'floors': floors}

    __all__ = ['DEFAULT_EMPTY_GAP_RATIO', 'DEFAULT_HARD_GAP_RATIO', '_decorate_candidates', '_flatten_candidates', '_insertion_counterfactuals', '_route_with_values']
    return dict(locals())
