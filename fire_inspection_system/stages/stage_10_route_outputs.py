...
def _build_s10_control():
    ...
    def build_locked_visit_plan(optimized_order_path: Path | str, candidates_path: Path | str, output_path: Path | str | None=None, floors: Iterable[str] | None=None) -> dict[str, Any]:
        ...
        for floor_id, route in sorted((optimized.get('floors') or {}).items()):
            ...
            for segment_index, raw_segment in enumerate(route.get('route_segments', []) or [], 1):
                ...
                if not segment_order:
                    continue
                route_segments.append({'segment_id': segment_id, 'segment_index': len(route_segments) + 1, 'entry_mode': text(raw_segment.get('entry_mode')), 'virtual_entry_id': text(raw_segment.get('virtual_entry_id')), 'physical_component_id': raw_segment.get('physical_component_id'), 'ordered_target_ids': segment_order})
            ...
            if not route_segments:
                ...
                route_segments = [{'segment_id': segment_id, 'segment_index': 1, 'entry_mode': 'single_open_route', 'virtual_entry_id': '', 'physical_component_id': route.get('component_id'), 'ordered_target_ids': list(order)}]
        ...
        payload = {'schema_version': 1, 'artifact_type': 'locked_control_visit_plan', 'order_locked': True, 'cost_recalculation_allowed': False, 'source_optimized_order': str(optimized_path), 'source_candidates': str(candidate_file), 'floors': result_floors}
...
def _build_s10_forwarding():
    ...
    def forward_locked_visit_plan(visit_plan: dict[str, Any], refined_graph: RefinedPhysicalGraph, free_spaces: VectorFreeSpaceIndex, cached_routes: dict[str, Any] | None=None) -> dict[str, Any]:
        ...
        return {'schema_version': 1, 'artifact_type': 'locked_order_temporal_forwarding_route', 'policy': {'primary_forwarding_plane': 'refined_physical_navigation_graph', 'fallback': 'none', 'route_type': 'physical_graph_walk', 'target_order_locked': True, 'control_cost_recalculated': False, 'support_degree_limit': None, 'structural_in_degree_limit': None, 'structural_out_degree_limit': None, 'repeated_physical_nodes_and_edges_allowed': True, 'repeated_traversals_have_temporal_event_order': True}, 'floors': floors, 'counts': {'floor_count': len(floors), 'feasible_floor_count': sum((bool(row.get('feasible')) for row in floors.values())), 'target_count': sum((int(row['counts']['target_count']) for row in floors.values())), 'target_visit_count': sum((int(row['counts']['target_visit_count']) for row in floors.values())), 'repeated_target_visit_count': sum((int(row['counts']['repeated_target_visit_count']) for row in floors.values())), 'route_segment_count': sum((int(row['counts']['route_segment_count']) for row in floors.values())), 'virtual_entry_count': sum((int(row['counts']['virtual_entry_count']) for row in floors.values())), 'graph_leg_count': sum((int(row['counts']['graph_leg_count']) for row in floors.values())), 'local_raster_leg_count': sum((int(row['counts']['local_raster_leg_count']) for row in floors.values())), 'repeated_physical_node_count': sum((int(row['counts']['repeated_physical_node_count']) for row in floors.values())), 'repeated_physical_edge_count': sum((int(row['counts']['repeated_physical_edge_count']) for row in floors.values()))}}
...
def _build_s10_acceptance():
    ...
    def write_acceptance_reports(output_dir: Path | str, visit_plan: dict[str, Any], forwarding: dict[str, Any]) -> dict[str, str]:
        ...
        for floor_id, route in sorted((forwarding.get('floors') or {}).items()):
            ...
            floor_reports[floor_id] = {'acceptance': 'passed' if route.get('feasible') else 'failed_with_diagnostics', 'status': route.get('status'), 'target_count': int(counts.get('target_count') or 0), 'expected_leg_count': int(counts.get('expected_leg_count') or 0), 'completed_leg_count': int(counts.get('completed_leg_count') or 0), 'failed_leg_count': int(counts.get('failed_leg_count') or 0), 'graph_leg_count': int(counts.get('graph_leg_count') or 0), 'local_raster_leg_count': int(counts.get('local_raster_leg_count') or 0), 'repeated_physical_node_count': int(counts.get('repeated_physical_node_count') or 0), 'repeated_physical_edge_count': int(counts.get('repeated_physical_edge_count') or 0), 'support_topology': topology, 'failed_legs': failures}
...
def _build_s10_annotated():
    ...
    def forwarding_to_geojson(forwarding: dict[str, Any], visit_plan: dict[str, Any]) -> dict[str, Any]:
        ...
        for floor_id, floor in sorted((forwarding.get('floors') or {}).items()):
            ...
            for target in floor_plan.get('targets', []) or []:
                ...
                features.append({'type': 'Feature', 'properties': {'feature_type': 'selected_inspection_target', 'floor_id': floor_id, 'target_id': target.get('target_id'), 'visit_order': target.get('visit_order'), 'route_segment_id': target.get('route_segment_id'), 'class_name': target.get('class_name'), 'raw_name': target.get('raw_name'), 'access_node_id': target.get('access_node_id'), 'virtual_access': target.get('virtual_access')}, 'geometry': {'type': 'Point', 'coordinates': list(point[:2])}})
    ...
    def write_inspection_visit_order(output_dir: Path | str, visit_plan: dict[str, Any]) -> dict[str, str]:
        ...
        with csv_path.open('w', encoding='utf-8-sig', newline='') as handle:
            fields = ['floor_id', 'route_segment_id', 'route_visit_order', 'first_visit_order', 'visit_ordinal_for_target', 'is_repeated_target_visit', 'target_id', 'class_name', 'raw_name', 'mandatory', 'access_node_id', 'x', 'y']
            writer = csv.DictWriter(handle, fieldnames=fields, extrasaction='ignore')
            writer.writeheader()
            writer.writerows(rows)
