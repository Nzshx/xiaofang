...
def _build_s10_annotated():
    ...
    def write_annotated_route_dxf(source_dxf: Path | str | None, output_dxf: Path | str, visit_plan: dict[str, Any], forwarding: dict[str, Any], *, original_obstacle_paths: Iterable[Path | str]=(), approved_repair_path: Path | None=None) -> Path:
    ...
    def build_last_inspection_route(run_dir: Path | str, optimized_order_path: Path | str, candidates_path: Path | str, effective_free_areas_path: Path | str, *, output_dir: Path | str | None=None, refined_graph_path: Path | str | None=None, source_dxf: Path | str | None=None, floors: Iterable[str] | None=None, route_cache_path: Path | str | None=None, max_grid_cells: int=1500000, write_dxf: bool=True) -> dict[str, Any]:
        ...
        if write_dxf:
            try:
                ...
                try:
                    dxf_path = write_annotated_route_dxf(source, preferred_path, visit_plan, forwarding, original_obstacle_paths=original_obstacle_paths, approved_repair_path=repair_path)
                except PermissionError:
                    version = time.strftime('%Y%m%d_%H%M%S')
                    dxf_path = write_annotated_route_dxf(source, output / f'{stem}_final_safe_route_{version}.dxf', visit_plan, forwarding, original_obstacle_paths=original_obstacle_paths, approved_repair_path=repair_path)
