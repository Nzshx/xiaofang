...
# -----------------------------------------------------------------------------
# Migrated implementation: scripts/detect_dxf_sheets_floors.py
# -----------------------------------------------------------------------------
def _build_s03_detector():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'scripts/detect_dxf_sheets_floors.py'
    )
    __name__ = 'scripts.detect_dxf_sheets_floors'
    __package__ = 'scripts'
    """
    DXF 图幅与楼层识别脚本。
    功能边界：
    ...
    6. 输出 drawing_sheets_floors.json、CSV 和 inspection region 审核 DXF。
    """
    import argparse
    import csv
    import json
    ...
    AXIS_DIMENSION_TERMS = ['轴', '轴线', '轴网', '轴号', '定位轴', '尺寸', '总尺寸', '标注', 'DIM', 'DIMS', 'DIMENSION', 'AXIS', 'GRID', 'A-AXIS', 'ADIM']
    AXIS_LABEL_CONTEXT_TERMS = ['轴', '轴文', '轴号', 'AXIS', 'AXIS_TEXT', '_AXISO']
    NON_AXIS_LABEL_PREFIXES = {'DN', 'DE', 'EL', 'FL', 'WL', 'HDZ', 'XF', 'FH', 'FM', 'JS', 'PS', 'PY', 'KT', 'EQ'}
    INSPECTION_REGION_EXCLUDE_TERMS = ['图框', '图幅', '图签', '标题栏', '图例', '说明', '目录', '材料表', '设备表', '剖面', '断面', '详图', '节点', '大样', '系统图', '原理图', '示意图', 'SHEET', 'FRAME', 'BORDER', 'TITLE', 'TK_LABEL', 'PMSHEET', 'LEGEND', 'NOTE', 'TABLE']
    AXIS_DIMENSION_STRICT_TERMS = ['轴', '轴线', '轴网', '轴号', '轴文', '定位轴', '尺寸', '总尺寸', '标注', '平时标注', 'AXIS', 'AXIS_TEXT', 'GRID', 'DIM', 'DIMS', 'DIMENSION', 'DIM_SYMB', 'DIM_ELEV', 'DIM_IDEN', 'PUB_DIM']
    INSPECTION_REGION_STRICT_EXCLUDE_TERMS = ['图框', '图幅', '图签', '标题栏', '图例', '说明', '目录', '材料表', '设备表', '剖面', '断面', '详图', '节点', '大样', '系统图', '原理图', '示意图', 'SHEET', 'FRAME', 'BORDER', 'TITLE', 'TK_LABEL', 'PMSHEET', 'LEGEND', 'NOTE', 'TABLE']
    FRAME_LAYER_TERMS = ['图框', '图幅', '标题栏', '图签', 'BORDER', 'FRAME', 'SHEET', 'TITLE', 'TK_LABEL', 'PMSHEET']
    FRAME_NEGATIVE_LAYER_TERMS = ['墙', '柱', '门', '窗', '填充', '家具', '设备', '洁具', '楼梯', '栏杆', '车位', '道路', '红线', '防火分区', 'HATCH', 'WALL', 'COLU', 'DOOR', 'STAIR', 'CAR']
    NON_PLAN_TEXT_TERMS = ['详图', '大样', '图例', '说明', '目录', '材料表', '设备表', '指标表', '剖面', '断面', '节点']
    FLOOR_CONTEXT_TERMS = ['平面', '平面图', '防火分区', '消防', '车库', '建筑', '地库']
    PLAN_TITLE_TERMS = ['平面图', '平面', '防火分区图', '消防平面', '地下室平面', '车库平面', '建筑平面']
    TITLEBAR_META_TERMS = ['PUB_TITLE', 'TK_LABEL', 'PMSHEET', 'TITLE', '图签', '标题栏', '图名', 'DRAWING TITLE']
    LOCAL_DETAIL_PLAN_TITLE_TERMS = ['详图', '节点', '大样', '局部', '检修口', '接头井', '风井', '排风井', '补风井', '楼梯', '坡道', '口部', '剖面', '断面', '立面']
    TITLE_NEGATIVE_PHRASES = ['详见平面图', '见平面图', '参见平面图', '详平面图']
    NON_PLAN_TITLE_TERMS = ['剖面图', '剖面', '刨面图', '刨面', '断面图', '断面', '立面图', '立面', '详图', '节点图', '节点', '大样图', '大样', '图例', '系统图', '原理图', '示意图']
    CN_NUMBERS = {'零': 0, '〇': 0, '一': 1, '二': 2, '两': 2, '三': 3, '四': 4, '五': 5, '六': 6, '七': 7, '八': 8, '九': 9}
    OUTPUT_FIELDS = ['sheet_id', 'floor_id', 'floor_name', 'sheet_title', 'floor_source', 'floor_confidence', 'sheet_semantic_role', 'sheet_semantic_name', 'sheet_semantic_confidence', 'path_planning_usable', 'sheet_role', 'needs_floor_review', 'method', 'sheet_confidence', 'object_count', 'text_count', 'seed_object_id', 'seed_text', 'seed_floor_id', 'seed_floor_name', 'seed_floor_evidence', 'seed_floor_confidence', 'growth_rounds', 'growth_object_count', 'growth_stop_reason', 'excluded_non_plan', 'non_plan_reason', 'dedup_floor_group_size', 'dedup_selected_reason', 'bbox_minx', 'bbox_miny', 'bbox_maxx', 'bbox_maxy', 'inspection_region_source', 'inspection_region_confidence', 'inspection_region_object_count', 'inspection_region_minx', 'inspection_region_miny', 'inspection_region_maxx', 'inspection_region_maxy', 'inspection_region_evidence', 'inspection_regions_json', 'evidence', 'semantic_evidence']
    ...
    def bbox_inflate(bbox: tuple[float, float, float, float], dx: float, dy: float | None=None) -> tuple[float, float, float, float]:
    ...
    def bbox_point_distance(bbox: tuple[float, float, float, float], point: tuple[float, float]) -> float:
    ...
    def bbox_clip(bbox: tuple[float, float, float, float], outer: tuple[float, float, float, float]) -> tuple[float, float, float, float] | None:
    ...
    def item_semantic_text(item: dict[str, Any]) -> str:
        ...
        return ' '.join((str(item.get(key, '') or '') for key in ('layer', 'parent_block_name', 'block_path', 'raw_text', 'norm_text', 'entity_type', 'geometry_kind')))
    ...
    def has_inspection_region_exclude_semantic(item: dict[str, Any]) -> bool:
        ...
        meta = item_semantic_text(item)
        return text_has_any(meta, INSPECTION_REGION_EXCLUDE_TERMS) or text_has_any(meta, INSPECTION_REGION_STRICT_EXCLUDE_TERMS)
    ...
    def is_line_like_item(item: dict[str, Any]) -> bool:
        ...
        return entity in {'LINE', 'LWPOLYLINE', 'POLYLINE', 'XLINE', 'RAY', 'ARC'} or 'line' in geometry or 'polyline' in geometry or ('curve' in geometry)
    ...
    def axis_label_group(item: dict[str, Any]) -> str:
        ...
        if not text or len(text) > 12:
            return ''
        match = re.fullmatch('([A-Z]{1,4})[- ]?([0-9]{1,3})([A-Z]?)', text)
        if match:
            prefix = match.group(1)
            return '' if prefix in NON_AXIS_LABEL_PREFIXES else prefix
        if re.fullmatch('[0-9]{1,3}', text):
            return 'NUM'
        return ''
    ...
    def loose_axis_label_group(item: dict[str, Any]) -> str:
        ...
        if text_has_any(meta, INSPECTION_REGION_EXCLUDE_TERMS):
            return ''
        text = normalize_axis_label_text(item.get('norm_text') or item.get('raw_text'))
        match = re.fullmatch('([A-Z])[- ]?([0-9]{1,3})([A-Z]?)', text)
        if not match:
            return ''
        prefix = match.group(1)
        return '' if prefix in NON_AXIS_LABEL_PREFIXES else prefix

    def axis_label_groups_in_sheet(sheet_bbox: tuple[float, float, float, float], items: list[dict[str, Any]], nearby_candidate_boxes: list[tuple[float, float, float, float]] | None=None) -> dict[str, list[tuple[float, float, float, float]]]:
        ...
        for item in items:
            ...
            if not group and nearby_candidate_boxes:
                ...
                if group:
                    ...
                    if not any((bbox_point_distance(candidate_box, center) <= loose_distance for candidate_box in nearby_candidate_boxes)):
                        group = ''
            if group:
                groups[group].append(clipped)
        return groups

    def axis_dimension_candidate_score(item: dict[str, Any], sheet_bbox: tuple[float, float, float, float]) -> tuple[float, list[str]]:
        bbox = item['bbox']
        clipped = bbox_clip(bbox, sheet_bbox)
        if clipped is None:
            return (0.0, [])
        ...
        edge_distance = min(abs(cx - sheet_bbox[0]) / sw, abs(cx - sheet_bbox[2]) / sw, abs(cy - sheet_bbox[1]) / sh, abs(cy - sheet_bbox[3]) / sh)
    ...
    def region_payload_from_boxes(*, region_id: str, boxes: list[tuple[float, float, float, float]], sheet_bbox: tuple[float, float, float, float], source: str, confidence: float, evidence: str, object_count: int) -> dict[str, Any] | None:
        ...
        if area_ratio < 0.02 or area_ratio > 0.97 or width_ratio < 0.12 or (height_ratio < 0.12):
            return None
        return {'region_id': region_id, 'bbox': region_bbox, 'source': source, 'confidence': round(confidence, 3), 'object_count': object_count, 'evidence': evidence}

    def layer_axis_dimension_envelope(*, sheet_bbox: tuple[float, float, float, float], candidates: list[tuple[tuple[float, float, float, float], float, list[str], str, dict[str, Any]]]) -> dict[str, Any] | None:
        if not candidates:
            return None
        sw = max(bbox_width(sheet_bbox), 1.0)
        sh = max(bbox_height(sheet_bbox), 1.0)
        ...
        grouped: dict[str, list[tuple[tuple[float, float, float, float], float, list[str], dict[str, Any]]]] = defaultdict(list)
        ...
        for layer, entries in grouped.items():
            ...
            if not (0.015 <= area_ratio <= 0.92 and width_ratio >= 0.1 and (height_ratio >= 0.1)):
                continue
            ...
            if not axis_layer and (not dim_layer):
                score -= 3.0
            layer_rows.append({'layer': layer, 'bbox': bbox, 'boxes': boxes, 'count': len(entries), 'line_count': line_count, 'text_count': text_count, 'insert_count': insert_count, 'horizontal_count': horizontal_count, 'vertical_count': vertical_count, 'area_ratio': area_ratio, 'width_ratio': width_ratio, 'height_ratio': height_ratio, 'axis_layer': axis_layer, 'dim_layer': dim_layer, 'score': score})
        if not layer_rows:
            return None
        ...
        if width_support < 0.55 or height_support < 0.45:
            return None
        evidence = ' | '.join((f"layer={row['layer']}:n={row['count']},score={row['score']:.1f},wr={row['width_ratio']:.2f},hr={row['height_ratio']:.2f}" for row in selected))
        return region_payload_from_boxes(region_id='R01', boxes=[selected_bbox], sheet_bbox=sheet_bbox, source='axis_dimension_layer_envelope', confidence=min(0.97, 0.72 + min(best['score'], 18.0) / 18.0 * 0.18), evidence=evidence, object_count=sum((row['count'] for row in selected)))

    def line_orientation_flags(bbox: tuple[float, float, float, float], sheet_bbox: tuple[float, float, float, float]) -> tuple[bool, bool]:
    ...
    def geometry_axis_dimension_layers(sheet_bbox: tuple[float, float, float, float], items: list[dict[str, Any]]) -> set[str]:
    ...
    def split_axis_dimension_regions_by_axis_labels(*, sheet_bbox: tuple[float, float, float, float], items: list[dict[str, Any]], candidates: list[tuple[tuple[float, float, float, float], float, list[str], str, dict[str, Any]]]) -> list[dict[str, Any]]:
        ...
        for group, boxes in loose_label_groups.items():
            ...
            edge_count = sum((1 for cx, cy in centers if cx <= sheet_bbox[0] + sw * 0.12 or cx >= sheet_bbox[2] - sw * 0.12 or cy <= sheet_bbox[1] + sh * 0.18 or (cy >= sheet_bbox[3] - sh * 0.18)))
        ...
        for group, payload in label_groups.items():
            ...
            usable_groups.append({'group': group, 'bbox': group_bbox, 'count': len(boxes), 'center': bbox_center(group_bbox), 'boxes': boxes, 'strict': bool(payload.get('strict')), 'edge_ratio': edge_ratio})
        ...
        for idx, group in enumerate(usable_groups, start=1):
            ...
            for bbox, _score, _reasons, _layer, _item in candidates:
                ...
                if low <= coord <= high:
                    ...
                    member_count += 1
            payload = region_payload_from_boxes(region_id=f"R{idx:02d}_{group['group']}", boxes=member_boxes, sheet_bbox=sheet_bbox, source=f"axis_label_group_{group['group']}", confidence=min(0.97, 0.72 + min(member_count, 160) / 160.0 * 0.2), evidence=f"axis_group={group['group']} axis={axis} labels={group['count']} candidates={member_count}", object_count=member_count)
            if payload:
                regions.append(payload)
        return regions if len(regions) >= 2 else []

    def infer_inspection_region(sheet: dict[str, Any], items: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        regions: list[dict[str, Any]] = [{'region_id': 'R01', 'bbox': sheet_bbox, 'source': source, 'confidence': confidence, 'object_count': 0, 'evidence': evidence}]
        if candidates:
            split_regions = split_axis_dimension_regions_by_axis_labels(sheet_bbox=sheet_bbox, items=items, candidates=candidates)
            if split_regions:
                ...
                if layer_payload:
                    ...
                    enough_envelope = len(candidates) >= 8 and 0.03 <= area_ratio <= 0.96 and (width_ratio >= 0.35) and (height_ratio >= 0.22)
                    if enough_envelope:
                        ...
                        evidence_parts = [f'{key}:{value}' for key, value in reason_counts.most_common(4)]
                        evidence_parts.extend((f'layer={key}:{value}' for key, value in layer_counts.most_common(3)))
                        source = 'axis_dimension_envelope'
                        confidence = min(0.96, 0.58 + min(len(candidates), 120) / 120.0 * 0.18 + min(width_ratio, 1.0) * 0.1 + min(height_ratio, 1.0) * 0.1)
                        evidence = ' | '.join(evidence_parts)
                        payload = region_payload_from_boxes(region_id='R01', boxes=candidate_boxes, sheet_bbox=sheet_bbox, source=source, confidence=confidence, evidence=evidence, object_count=len(candidates))
                        if payload:
                            regions = [payload]
                        ...
                        evidence = f'fallback: weak envelope objects={len(candidates)} area_ratio={area_ratio:.3f} width_ratio={width_ratio:.3f} height_ratio={height_ratio:.3f}'
        serialized_regions = [{**region, 'bbox': [region['bbox'][0], region['bbox'][1], region['bbox'][2], region['bbox'][3]]} for region in regions]
        return {'inspection_region_bbox': region_bbox, 'inspection_regions': serialized_regions, 'inspection_region_source': source, 'inspection_region_confidence': round(confidence, 3), 'inspection_region_object_count': len(candidates), 'inspection_region_evidence': evidence}
    ...
    def detect_floor_mentions(value: Any) -> list[tuple[str, str]]:
        ...
        for pattern in ['地下([一二两三四五六七八九十\\d]+)层', '负([一二两三四五六七八九十\\d]+)层']:
            for raw in re.findall(pattern, text):
                number = chinese_number_to_int(raw)
                if number:
                    matches.append((f'B{number}', f'地下{raw}层'))
        for raw in re.findall('\\bB\\s*([0-9]+)\\s*(?:F|层)?\\b', text, flags=re.I):
            number = as_int(raw)
            if number:
                matches.append((f'B{number}', f'B{number}'))
        for raw in re.findall('(?:第)?([一二两三四五六七八九十\\d]+)层', text):
            if re.search(f'(地下|负){re.escape(raw)}层', text):
                continue
            number = chinese_number_to_int(raw)
            if number:
                matches.append((f'F{number}', f'{raw}层'))
        for raw in re.findall('\\b([1-9][0-9]?)\\s*F\\b', text, flags=re.I):
            number = as_int(raw)
            if number:
    ...
    def floor_title_candidate_for_item(item: dict[str, Any]) -> tuple[str, list[tuple[str, str]], float] | None:
        ...
        if not mentions:
            return None
        meta = ' '.join([str(item.get('layer', '')), str(item.get('parent_block_name', '')), str(item.get('block_path', ''))])
        weight = 36.0
    ...
    def titlebar_meta_text(item: dict[str, Any]) -> str:
        ...
        return ' '.join([str(item.get('layer', '')), str(item.get('parent_block_name', '')), str(item.get('block_path', ''))])
    ...
    def is_titlebar_area_item(item: dict[str, Any], sheet_bbox: tuple[float, float, float, float], title: str) -> bool:
        ...
        if not text_is_short_title:
            return False
        right_title_zone = cx >= sheet_bbox[0] + sw * 0.7
        bottom_title_zone = cy <= sheet_bbox[1] + sh * 0.24
        return right_title_zone or bottom_title_zone

    def sheet_title_candidate_for_item(item: dict[str, Any], sheet_bbox: tuple[float, float, float, float]) -> tuple[str, list[tuple[str, str]], float] | None:
        ...
        with path.open(encoding='utf-8-sig', newline='') as f:
            for row in csv.DictReader(f):
                ...
                if not bbox:
                    continue
                cx, cy = bbox_center(bbox)
                items.append({'object_id': row.get('object_id', ''), 'source': row.get('source', ''), 'entity_type': row.get('entity_type', ''), 'geometry_kind': row.get('geometry_kind', ''), 'is_closed': row.get('is_closed', ''), 'layer': row.get('layer', ''), 'parent_block_name': row.get('parent_block_name', ''), 'block_path': row.get('block_path', ''), 'raw_text': row.get('raw_text', ''), 'norm_text': row.get('norm_text', ''), 'bbox': bbox, 'cx': cx, 'cy': cy, 'area': bbox_area(bbox)})
        return items
    ...
    def frame_candidate_score(item: dict[str, Any], global_bbox: tuple[float, float, float, float]) -> float:
        ...
        text = ' '.join([str(item.get('layer', '')), str(item.get('parent_block_name', '')), str(item.get('block_path', ''))])
        ...
        if area_ratio >= 0.015:
            score += 1.0
        if has_frame_semantic and area >= 1000000000.0:
            score += 2.0
        if has_frame_semantic and width >= 80000 and (height >= 50000):
            score += 1.0
        return score

    def collect_frame_title_evidence(frame_bbox: tuple[float, float, float, float], items: list[dict[str, Any]]) -> dict[str, Any]:
        """统计图框内部标题栏图名和楼层证据。"""
        ...
        return {'has_plan_title_evidence': title_count > 0, 'title_evidence_count': title_count, 'title_evidence_samples': title_samples, 'title_floor_ids': floor_ids[:8]}

    def detect_frame_sheets(items: list[dict[str, Any]], global_bbox: tuple[float, float, float, float]) -> list[dict[str, Any]]:
        ...
        for item in items:
            ...
            if score < 5.0:
                continue
            candidates.append({'bbox': item['bbox'], 'method': 'frame_candidate', 'confidence': min(0.95, 0.45 + score * 0.06), 'score': score, 'evidence': [f"entity={item.get('entity_type')}", f"geometry={item.get('geometry_kind')}", f"layer={item.get('layer')}", f"block={item.get('parent_block_name')}"]})
        if len(candidates) >= 10:
            areas = sorted((bbox_area(row['bbox']) for row in candidates))
            reference_area = areas[int(len(areas) * 0.75)]
            min_candidate_area = max(reference_area * 0.3, areas[-1] * 0.15)
        ...
        for cand in candidates:
            if any((bbox_iou(cand['bbox'], old['bbox']) >= 0.7 for old in kept)):
                continue
            ...
            if contained_by_existing:
                continue
            kept = [old for old in kept if not (bbox_area(old['bbox']) < cand_area and bbox_containment(old['bbox'], cand['bbox']) >= 0.88)]
            kept.append(cand)
            if len(kept) >= 80:
                break
        for cand in kept:
            ...
            if title_evidence['has_plan_title_evidence']:
                cand['evidence'].append('internal_plan_title=' + ','.join(title_evidence.get('title_evidence_samples', [])[:3]))
                cand['score'] += 4.0
    ...
    def select_cluster_items(items: list[dict[str, Any]], global_bbox: tuple[float, float, float, float]) -> list[dict[str, Any]]:
    ...
    def detect_cluster_sheets(items: list[dict[str, Any]], global_bbox: tuple[float, float, float, float], *, grid_size: int=180) -> list[dict[str, Any]]:
        ...
        for comp in components:
            ...
            bbox = bbox_union(boxes)
            area = bbox_area(bbox)
            if area < global_area * 0.002 or area > global_area * 0.9:
                continue
            sheets.append({'bbox': bbox, 'method': 'density_cluster', 'confidence': min(0.88, 0.45 + min(len(indices), 1000) / 2500), 'score': len(indices), 'evidence': [f'density_cluster_items={len(indices)}']})
    ...
    class QuadTreeNode:
        ...
        def __init__(self, bounds: tuple[float, float, float, float], *, depth: int=0, max_depth: int=9, max_points: int=80) -> None:
        ...
        def _split(self) -> None:
            ...
            if midx <= minx or midx >= maxx or midy <= miny or (midy >= maxy):
                return
            self.children = [QuadTreeNode((minx, miny, midx, midy), depth=self.depth + 1, max_depth=self.max_depth, max_points=self.max_points), QuadTreeNode((midx, miny, maxx, midy), depth=self.depth + 1, max_depth=self.max_depth, max_points=self.max_points), QuadTreeNode((minx, midy, midx, maxy), depth=self.depth + 1, max_depth=self.max_depth, max_points=self.max_points), QuadTreeNode((midx, midy, maxx, maxy), depth=self.depth + 1, max_depth=self.max_depth, max_points=self.max_points)]
    ...
    class QuadTreeIndex:
        ...
        def __init__(self, items: list[dict[str, Any]], bounds: tuple[float, float, float, float]) -> None:
    ...
    def plan_title_seed_for_item(item: dict[str, Any]) -> dict[str, Any] | None:
        ...
        return {'item': item, 'seed_object_id': item.get('object_id', ''), 'seed_text': title, 'floor_id': floor_id, 'floor_name': floor_label(floor_id), 'floor_evidence': floor_evidence, 'weight': weight, 'center': (float(item['cx']), float(item['cy']))}
    ...
    def containing_frame_for_seed(seed_center: tuple[float, float], seed_centers: list[tuple[float, float]], frame_sheets: list[dict[str, Any]], global_bbox: tuple[float, float, float, float]) -> dict[str, Any] | None:
    ...
    def grow_seed_bbox(seed: dict[str, Any], seed_index: int, seed_centers: list[tuple[float, float]], growth_items: list[dict[str, Any]], spatial_index: QuadTreeIndex, global_bbox: tuple[float, float, float, float]) -> tuple[tuple[float, float, float, float], int, int, str]:
        ...
        for round_idx in range(1, 15):
            ...
            if candidate_area > global_area * 0.7:
    ...
    def detect_plan_title_seed_growth_sheets(items: list[dict[str, Any]], global_bbox: tuple[float, float, float, float], frame_sheets: list[dict[str, Any]] | None=None) -> list[dict[str, Any]]:
        ...
        for seed_index, seed in enumerate(seeds):
            grown_bbox, object_count, rounds, stop_reason = grow_seed_bbox(seed, seed_index, seed_centers, growth_items, spatial_index, global_bbox)
            ...
            sheets.append({'bbox': final_bbox, 'method': method, 'confidence': round(min(0.94, confidence), 3), 'score': float(seed['weight']) + min(object_count, 5000) / 100.0, 'seed_object_id': seed['seed_object_id'], 'seed_text': seed['seed_text'], 'seed_cx': seed['center'][0], 'seed_cy': seed['center'][1], 'growth_rounds': rounds, 'growth_object_count': object_count, 'growth_stop_reason': stop_reason, 'excluded_non_plan': False, 'non_plan_reason': '', 'sheet_title': seed['seed_text'], 'seed_floor_id': seed['floor_id'], 'seed_floor_name': seed['floor_name'], 'seed_floor_evidence': seed['floor_evidence'], 'seed_floor_confidence': 0.82, 'evidence': [f"seed_object_id={seed['seed_object_id']}", f"seed_text={seed['seed_text']}", f"seed_floor={seed['floor_id']}", f'growth_object_count={object_count}', f'growth_stop_reason={stop_reason}']})
    ...
    def merge_sheet_candidates(frame_sheets: list[dict[str, Any]], seed_growth_sheets: list[dict[str, Any]], cluster_sheets: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        method_priority = {'frame_candidate': 3.0, 'plan_title_seed_region_growing_frame_fused': 2.8, 'plan_title_seed_region_growing': 2.5, 'density_cluster': 1.0}
        candidates.sort(key=lambda row: (method_priority.get(str(row.get('method', '')), 0.0), row.get('confidence', 0), bbox_area(row['bbox'])), reverse=True)
        seed_points = [(float(row.get('seed_cx')), float(row.get('seed_cy'))) for row in seed_growth_sheets if row.get('seed_cx') not in ('', None) and row.get('seed_cy') not in ('', None)]
        ...
        for cand in candidates:
            ...
            for old in kept:
                ...
                if cand_method.startswith('plan_title_seed') and old_method == 'frame_candidate':
                    ...
                    if old_seed_count <= 1 and (bbox_contains_point(old_bbox, cand_center) or bbox_containment(cand_bbox, old_bbox) >= 0.55 or overlap_share >= 0.1):
                        duplicate = True
                        break
            if duplicate:
                continue
    ...
    def dedup_sheets_by_floor_id(sheets: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int]:
        ...
        method_priority = {'frame_candidate': 100.0, 'plan_title_seed_region_growing_frame_fused': 85.0, 'plan_title_seed_region_growing': 72.0, 'density_cluster': 45.0}
        source_priority = {'sheet_title': 120.0, 'plan_title_seed': 110.0, 'full_sheet_vote': 55.0}
        selected: list[dict[str, Any]] = []
        removed_count = 0
        for floor_id, group in floor_groups.items():
            ...
            def score(sheet: dict[str, Any]) -> tuple[float, float, float, float]:
                ...
                total = usable_score + source_priority.get(floor_source, 0.0) + method_priority.get(method, 0.0) + min(object_count, 12000) / 120.0 + area_score + confidence * 80.0
    ...
    def infer_sheet_floor(sheet: dict[str, Any], items: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if title_scores:
            ...
            confidence = min(0.98, max(0.72, best_score / total * 0.62 + min(best_score / 80, 0.25) + min((best_score - second_score) / max(best_score, 1), 1.0) * 0.11))
            sheet_title = title_text_by_floor.get(best_floor, (0.0, ''))[1]
            return {'floor_id': best_floor, 'floor_name': floor_label(best_floor), 'floor_confidence': round(confidence, 3), 'floor_source': 'sheet_title', 'sheet_title': sheet_title, 'floor_candidates': [{'floor_id': floor_id, 'floor_name': floor_label(floor_id), 'score': round(score, 2), 'evidence': title_evidence_by_floor.get(floor_id, [])[:6]} for floor_id, score in ranked[:6]], 'floor_evidence': title_evidence_by_floor.get(best_floor, [])[:8], 'object_count': object_count, 'text_count': text_count}
        return {'floor_id': '', 'floor_name': '', 'floor_confidence': 0.0, 'floor_source': '', 'sheet_title': '', 'floor_candidates': [], 'floor_evidence': ['missing_titlebar_plan_title'] if sheet.get('method') == 'frame_candidate' else [], 'object_count': object_count, 'text_count': text_count}
    ...
    def semantic_values_for_item(item: dict[str, Any]) -> list[tuple[str, str, float]]:
        ...
        return [('text', str(raw_text or ''), 4.0), ('parent_block', str(item.get('parent_block_name') or ''), 2.5), ('block_path', str(item.get('block_path') or ''), 2.0), ('layer', str(item.get('layer') or ''), 1.2)]
    ...
    def infer_sheet_semantic(sheet: dict[str, Any], items: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in items:
            ...
            if not item_has_titlebar_meta(item) and (not (text_title and is_titlebar_area_item(item, bbox, text_title))):
                continue
        ...
        if floor_confidence >= 0.9:
            return {'sheet_semantic_role': 'path_plan_floor_plan', 'sheet_semantic_name': 'selected_by_high_floor_confidence', 'sheet_semantic_confidence': floor_confidence, 'path_planning_usable': True, 'sheet_semantic_evidence': plan_evidence[:8] or sheet.get('floor_evidence', [])[:4]}
        if floor_confidence <= 0.0 or not floor_id or object_count < 500 or (text_count < 20):
            return {'sheet_semantic_role': 'excluded_zero_confidence', 'sheet_semantic_name': 'excluded_by_zero_or_invalid_floor_confidence', 'sheet_semantic_confidence': 0.0, 'path_planning_usable': False, 'sheet_semantic_evidence': []}
        ...
        if has_strong_plan_title or (has_plan_title and (not non_plan_evidence)):
            return {'sheet_semantic_role': 'path_plan_floor_plan', 'sheet_semantic_name': 'selected_by_plan_title', 'sheet_semantic_confidence': round(min(0.89, 0.45 + floor_confidence * 0.25 + min(plan_score / 25.0, 0.18)), 3), 'path_planning_usable': True, 'sheet_semantic_evidence': strong_plan_evidence[:8] or plan_evidence[:8]}
        return {'sheet_semantic_role': 'excluded_non_plan_or_no_plan_title', 'sheet_semantic_name': 'excluded_by_non_plan_or_missing_plan_title', 'sheet_semantic_confidence': 0.0, 'path_planning_usable': False, 'sheet_semantic_evidence': non_plan_evidence[:8] or plan_evidence[:8]}
    ...
    def output_rows(sheets: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for idx, sheet in enumerate(sheets, start=1):
            ...
            try:
                region_minx, region_miny, region_maxx, region_maxy = (float(raw_region_bbox[0]), float(raw_region_bbox[1]), float(raw_region_bbox[2]), float(raw_region_bbox[3]))
            except Exception:
            ...
            rows.append({'sheet_id': f'SHEET_{idx:03d}', 'floor_id': floor_id, 'floor_name': sheet.get('floor_name', ''), 'sheet_title': sheet.get('sheet_title', ''), 'floor_source': sheet.get('floor_source', ''), 'floor_confidence': floor_confidence, 'sheet_semantic_role': semantic_role, 'sheet_semantic_name': semantic_name, 'sheet_semantic_confidence': semantic_confidence, 'path_planning_usable': path_planning_usable, 'sheet_role': 'path_planning_floor_plan' if path_planning_usable else semantic_role, 'needs_floor_review': path_planning_usable and floor_confidence < 0.75, 'method': sheet.get('method', ''), 'sheet_confidence': round(float(sheet.get('confidence', 0.0)), 3), 'object_count': object_count, 'text_count': text_count, 'seed_object_id': sheet.get('seed_object_id', ''), 'seed_text': sheet.get('seed_text', ''), 'seed_floor_id': sheet.get('seed_floor_id', ''), 'seed_floor_name': sheet.get('seed_floor_name', ''), 'seed_floor_evidence': sheet.get('seed_floor_evidence', ''), 'seed_floor_confidence': sheet.get('seed_floor_confidence', ''), 'growth_rounds': sheet.get('growth_rounds', ''), 'growth_object_count': sheet.get('growth_object_count', ''), 'growth_stop_reason': sheet.get('growth_stop_reason', ''), 'excluded_non_plan': sheet.get('excluded_non_plan', False), 'non_plan_reason': sheet.get('non_plan_reason', ''), 'dedup_floor_group_size': sheet.get('dedup_floor_group_size', ''), 'dedup_selected_reason': sheet.get('dedup_selected_reason', ''), 'bbox_minx': minx, 'bbox_miny': miny, 'bbox_maxx': maxx, 'bbox_maxy': maxy, 'inspection_region_source': sheet.get('inspection_region_source', ''), 'inspection_region_confidence': sheet.get('inspection_region_confidence', ''), 'inspection_region_object_count': sheet.get('inspection_region_object_count', ''), 'inspection_region_minx': region_minx, 'inspection_region_miny': region_miny, 'inspection_region_maxx': region_maxx, 'inspection_region_maxy': region_maxy, 'inspection_region_evidence': sheet.get('inspection_region_evidence', ''), 'inspection_regions_json': json.dumps(sheet.get('inspection_regions', []), ensure_ascii=False), 'evidence': ' | '.join(sheet.get('floor_evidence', [])[:6]), 'semantic_evidence': ' | '.join(sheet.get('sheet_semantic_evidence', [])[:6])})
        return rows
    ...
    def inspection_region_bboxes_from_row(row: dict[str, Any]) -> list[BBox]:
        ...
        if isinstance(direct_bbox, (list, tuple)) and len(direct_bbox) >= 4:
            try:
                boxes.append((float(direct_bbox[0]), float(direct_bbox[1]), float(direct_bbox[2]), float(direct_bbox[3])))
            except (TypeError, ValueError):
                pass
        keys = ('inspection_region_minx', 'inspection_region_miny', 'inspection_region_maxx', 'inspection_region_maxy')
        if all((row.get(key) not in ('', None) for key in keys)):
            try:
                boxes.append(tuple((float(row[key]) for key in keys)))
            except (TypeError, ValueError):
                pass
        unique: list[BBox] = []
    ...
    def write_overlay_dxf(input_dxf: Path, output_dxf: Path, rows: list[dict[str, Any]], *, include_non_path: bool=False) -> None:
        ...
        for row in rows:
            ...
            label_parts = [str(row.get('sheet_id') or ''), str(row.get('floor_name') or row.get('floor_id') or row.get('sheet_title') or 'UNKNOWN'), str(row.get('sheet_semantic_role') or '')]
            label = ' '.join((part for part in label_parts if part)).strip()[:180]
            if label:
                text = msp.add_text(label, dxfattribs={'layer': SHEET_LABEL_LAYER, 'color': sheet_color, 'height': height})
            ...
            if usable:
                for idx, region_bbox in enumerate(inspection_region_bboxes_from_row(row), start=1):
                    ...
                    region_text = msp.add_text(f"{row.get('sheet_id') or ''} region {idx}", dxfattribs={'layer': INSPECTION_REGION_LABEL_LAYER, 'color': 1, 'height': max((rmaxy - rminy) * 0.015, 180.0)})
    ...
    def detect_sheets_and_floors(inventory_dir: Path, *, grid_size: int=180) -> dict[str, Any]:
        ...
        return {'inventory_dir': str(inventory_dir.resolve()), 'global_bbox': global_bbox, 'item_count': len(items), 'frame_candidate_count': len(frame_sheets), 'plan_title_seed_candidate_count': len(seed_growth_sheets), 'cluster_candidate_count': len(cluster_sheets), 'floor_dedup_removed_count': floor_dedup_removed_count, 'sheet_count': len(enriched), 'floor_plan_candidate_count': usable_count, 'unknown_or_non_floor_count': semantic_counts.get('unknown_or_non_floor', 0), 'path_planning_usable_count': usable_count, 'needs_floor_review_count': review_count, 'floor_counts': dict(floor_counts), 'path_planning_floor_counts': dict(usable_floor_counts), 'sheet_semantic_counts': dict(semantic_counts), 'sheets': [{**row, 'bbox': [row['bbox_minx'], row['bbox_miny'], row['bbox_maxx'], row['bbox_maxy']], 'inspection_region_bbox': [row['inspection_region_minx'], row['inspection_region_miny'], row['inspection_region_maxx'], row['inspection_region_maxy']], 'inspection_regions': enriched[idx].get('inspection_regions', []), 'floor_candidates': enriched[idx].get('floor_candidates', []), 'sheet_evidence': enriched[idx].get('evidence', []), 'sheet_semantic_evidence': enriched[idx].get('sheet_semantic_evidence', [])} for idx, row in enumerate(rows)]}

    return dict(locals())

_s03_detector = _register_embedded_module(
    'scripts.detect_dxf_sheets_floors',
    _build_s03_detector(),
    aliases=('detect_dxf_sheets_floors',),
)

# -----------------------------------------------------------------------------
# Migrated implementation: fire_inspection_system/cad_preprocessing.py
# -----------------------------------------------------------------------------
def _build_s03_api():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'fire_inspection_system/cad_preprocessing.py'
    )
    __name__ = 'fire_inspection_system.cad_preprocessing'
    ...
    from detect_dxf_sheets_floors import detect_sheets_and_floors, unique_output_path, write_csv, write_overlay_dxf
    ...
    def preprocess_cad_drawing(input_dxf: Path | str, inventory_dir: Path | str, output_dir: Path | str, *, grid_size: int=180, write_floor_overlay: bool=True, overlay_all_sheets: bool=False) -> CadPreprocessResult:
        ...
        if write_floor_overlay:
            ...
            write_overlay_dxf(input_path, overlay_path, result['sheets'], include_non_path=overlay_all_sheets)
        ...
        return CadPreprocessResult(sheets_json=sheets_json, sheets_csv=sheets_csv, overlay_dxf=overlay_path, sheet_count=int(result.get('sheet_count') or len(result['sheets'])), floor_count=len(floor_ids), usable_region_count=int(result.get('path_planning_usable_count') or len(usable_floors)), floors=usable_floors)
    return dict(locals())

_s03_api = _register_embedded_module(
    'fire_inspection_system.cad_preprocessing',
    _build_s03_api(),
    aliases=('cad_preprocessing',),
)
...
def run_stage(
    ...
    *,
    grid_size: int,
    write_floor_overlay: bool,
) -> Any:
    return _s03_api.preprocess_cad_drawing(
        input_dxf,
        inventory_dir,
        run_dir / "preprocess",
        grid_size=grid_size,
        write_floor_overlay=write_floor_overlay,
    )
