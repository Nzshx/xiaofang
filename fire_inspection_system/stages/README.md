...
系统只生成阶段 10 的 `*_final_safe_route.dxf`。该文件在原始 CAD 上统一叠加已识别障碍物、已批准拓扑补墙、最终安全路线和巡检访问序号；楼层框、巡检对象、单独障碍物、单独补墙、AreaGraph、规则导航图及 Connector 修正均不再生成独立 DXF。JSON、CSV、GeoJSON、PNG 和验收报告仍照常输出。
