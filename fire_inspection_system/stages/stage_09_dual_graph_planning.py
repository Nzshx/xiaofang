...
def _build_s09_planner():
    ...
    def _minimum_area_tree(value_graph: Mapping[str, Any], area_values: Mapping[str, float], config: DualGraphConfig) -> tuple[str, list[dict[str, Any]], list[str]]:
        areas = sorted(area_values)
        if not areas:
            return ('', [], [])
        root = max(areas, key=lambda area: (area_values[area], area))
    ...
    def _plan_floor(floor_id: str, floor: Mapping[str, Any], closure: PhysicalMetricClosure, recommendation: Mapping[str, Any], recommendation_key: str, config: DualGraphConfig) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        for segment_index, cluster in enumerate(selected_clusters, 1):
            ...
            segment_graph, targets_by_area, area_values = _area_graph(required, filtered_candidates, view, floor_id, closure, config)
            root, area_tree, unreachable_areas = _minimum_area_tree(segment_graph, area_values, config)
            ...
            support_targets, target_tree, segment_dmax_audit = _connect_required_targets_with_dmax_relays(required, cluster, root_target, floor_id, closure, view, filtered_candidates, dmax)
            ...
            for relay in sorted(support_targets - required):
                selection_reason[relay] = 'dmax_rgcn_scored_relay'
            ...
            route_segments.append({'segment_id': segment_id, 'physical_component_id': physical_component_id, 'entry_mode': 'independent_open_route_virtual_entry', 'virtual_entry_id': f'VIRTUAL_ENTRY::{segment_id}', 'entry_target_id': root_target, 'ordered_target_ids': local_order, 'first_visit_order': _first_visits(local_order), 'required_target_ids': sorted(required), 'support_target_ids': sorted(support_targets), 'relay_target_ids': sorted(support_targets - required), 'open_endpoint_target_id': endpoint, 'length': sum(local_distances), 'maximum_continuous_uninspected_distance': max(local_distances, default=0.0), 'dmax_constraint_satisfied': all((value <= dmax + EPS for value in local_distances)), 'root_high_value_area_id': root, 'area_visit_order': local_area_order, 'area_backbone_tree': area_tree, 'target_backbone_tree': target_tree})
    ...
    def build_dual_graph_inspection_route(run_dir: Path | str, candidates_path: Path | str, effective_free_areas_path: Path | str, refined_graph_path: Path | str, output_dir: Path | str, *, recommendations_path: Path | str | None=None, source_run_id: str='', source_dxf: Path | str | None=None, config: DualGraphConfig | None=None) -> dict[str, Any]:
        ...
        for floor_id, floor in sorted((candidates_bundle.get('floors') or {}).items()):
            ...
            if floor_id not in closure.floor_ids():
                optimized_floors[floor_id] = {'floor_id': floor_id, 'status': 'infeasible_missing_physical_closure', 'feasible': False, 'reason': 'floor_missing_from_precomputed_physical_distance_matrix', 'order': [], 'selected_target_ids': [], 'solver': 'dual_graph_rgcn_area_tree_tsp'}
                continue
            recommendation, recommendation_key = _recommendation_payload(recommendations, source_run_id, floor_id)
            plan, graph = _plan_floor(floor_id, floor, closure, recommendation, recommendation_key, cfg)
            optimized_floors[floor_id] = plan
            value_graph_floors[floor_id] = graph
        planning_seconds = time.perf_counter() - planning_started
        optimized_path = _write(output / 'optimized_target_order.json', {'schema_version': 1, 'architecture': 'rgcn_value_graph_plus_certified_physical_graph', 'route_type': 'area_forest_supported_open_physical_walks_with_dmax', 'same_floor_only': True, 'cross_segment_physical_jumps_allowed': False, 'floors': optimized_floors})
