...
def _build_s05_obstacles():
    ...
    def load_floor_regions(sheets_json: Path) -> list[dict[str, Any]]:
        ...
        for sheet in payload.get('sheets', []) or []:
            ...
            for index, item in enumerate(inspection_regions, start=1):
                ...
                regions.append({'sheet_id': sheet_id, 'floor_id': floor_id, 'floor_name': floor_name, 'region_id': region_id, 'full_region_id': f'{sheet_id}:{region_id}', 'bbox': bbox_tuple, 'polygon': box(*bbox_tuple), 'region_source': source, 'region_confidence': confidence, 'region_evidence': str(item.get('evidence') or sheet.get('inspection_region_evidence') or '')})
        return regions
    ...
    def write_obstacle_csv(path: Path, obstacles: list[dict[str, Any]]) -> None:
        fields = ['obstacle_id', 'object_id', 'handle', 'source', 'sheet_id', 'floor_id', 'floor_name', 'region_id', 'full_region_id', 'obstacle_type', 'reason', 'confidence', 'layer', 'entity_type', 'geometry_kind', 'color', 'linetype', 'parent_block_name', 'semantic_evidence', 'geometry_evidence', 'topology_evidence', 'negative_evidence', 'fusion_decision', 'bbox_minx', 'bbox_miny', 'bbox_maxx', 'bbox_maxy', 'area']
        path.parent.mkdir(parents=True, exist_ok=True)
