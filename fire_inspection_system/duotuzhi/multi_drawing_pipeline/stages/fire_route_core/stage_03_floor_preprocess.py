...
def _build_s03_detector():
    ...
    6. 输出 drawing_sheets_floors.json 和 CSV。
    """
    import argparse
    import csv
    import json
...
# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/cad_preprocessing.py
# -----------------------------------------------------------------------------
def _build_s03_api():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/cad_preprocessing.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.cad_preprocessing'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core'
    import json
    import sys
    ...
    from detect_dxf_sheets_floors import detect_sheets_and_floors, write_csv
    ...
    def preprocess_cad_drawing(input_dxf: Path | str, inventory_dir: Path | str, output_dir: Path | str, *, grid_size: int=180) -> CadPreprocessResult:
        ...
        return CadPreprocessResult(sheets_json=sheets_json, sheets_csv=sheets_csv, sheet_count=int(result.get('sheet_count') or len(result['sheets'])), floor_count=len(floor_ids), usable_region_count=int(result.get('path_planning_usable_count') or len(usable_floors)), floors=usable_floors)
    return dict(locals())

_s03_api = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.cad_preprocessing',
    _build_s03_api(),
