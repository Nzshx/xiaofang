...
from multi_drawing_pipeline.stages.fire_route_core import stage_06_navigation_graph as _stage06

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/connector_portal_refinement.py
# -----------------------------------------------------------------------------
def _build_s07_connector():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/connector_portal_refinement.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.connector_portal_refinement'
    ...
    * every ordinary/refinement edge remains strictly inside the original free
    ...
    class RefinementConfig:
        ...
        door_supported_gap_pixels: float = 4.5
        door_evidence_radius_pixels: float = 2.5
        door_context_radius_pixels: float = 4.0
        require_confirmed_door_evidence: bool = True
    ...
    def _load_opening_evidence(run_dir: Path, pixel_size_by_floor: dict[str, float], source_dxf: Path | str | None=None) -> tuple[dict[str, list[dict[str, Any]]], dict[str, int]]:
        from multi_drawing_pipeline.stages.fire_route_core import stage_05B_raw_cad_collision as raw_cad_safety
        ...
        evidence_by_floor: dict[str, list[dict[str, Any]]] = defaultdict(list)
        entity_counts: Counter[str] = Counter()
        for row in rows:
            layer = str(row.get('layer') or '')
            parent_block = str(row.get('parent_block_name') or '')
            evidence_text = f'{layer} {parent_block}'.upper()
            window_like = any(token in evidence_text for token in ('WIND', 'WINDOW', 'GLZ', 'GLASS', 'CURTAIN', '窗', '玻璃', '幕墙'))
            semantic_door = layer_has_opening_semantics(evidence_text) and (not window_like)
            ...
            pixel_size = float(pixel_size_by_floor.get(floor_id) or 100.0)
            width = float(bounds[2] - bounds[0])
            height = float(bounds[3] - bounds[1])
            arc_size = max(width, height)
            geometric_door = (
                entity_type in {'ARC', 'CIRCLE'}
                and 2.5 * pixel_size <= arc_size <= 24.0 * pixel_size
                and min(width, height) >= pixel_size
            )
            if not semantic_door and not geometric_door:
                continue
            if not geometries:
                geometries = [Point(center_x, center_y)]
            evidence_source = 'inventory_door_semantics' if semantic_door else 'inventory_door_arc'
            evidence_by_floor[floor_id].extend(
                {'geometry': geometry, 'evidence_source': evidence_source, 'confidence': 0.9}
                for geometry in geometries
            )
            entity_counts[floor_id] += 1
        direct_dxf_evidence = raw_cad_safety.extract_dxf_door_swing_evidence(
            run_dir,
            source_dxf=source_dxf,
        )
        for feature in _read_json(direct_dxf_evidence).get('features', []) or []:
            properties = dict(feature.get('properties') or {})
            floor_id = str(properties.get('floor_id') or '')
            geometry_payload = feature.get('geometry')
            if not floor_id or not geometry_payload:
                continue
            evidence_by_floor[floor_id].append(
                {
                    'geometry': shape(geometry_payload),
                    'evidence_source': str(properties.get('evidence_source') or 'dxf_recursive_door_swing_arc'),
                    'confidence': float(properties.get('confidence') or 0.93),
                    'source_handle': str(properties.get('source_handle') or ''),
                    'radius': properties.get('radius'),
                    'sweep_degrees': properties.get('sweep_degrees'),
                }
            )
            entity_counts[floor_id] += 1
        return (dict(evidence_by_floor), dict(entity_counts))
    ...
    def _connector_candidates(floor_id: str, used_components: Counter[str], geometries: dict[str, Polygon], opening_evidence: list[dict[str, Any]] | None, pixel_size: float, config: RefinementConfig) -> list[dict[str, Any]]:
        ...
        for index, component_a in enumerate(component_ids):
            ...
            for component_b in component_ids[index + 1:]:
                ...
                door_rows = list(opening_evidence or [])
                for door_index, evidence in enumerate(door_rows, 1):
                    door_geometry = evidence.get('geometry')
                    if door_geometry is None or door_geometry.is_empty:
                        continue
                    radius_pixels = float(evidence.get('radius') or 0.0) / max(pixel_size, 1e-09)
                    context_radius_pixels = min(16.0, max(config.door_context_radius_pixels, radius_pixels + 2.0))
                    context = door_geometry.buffer(pixel_size * context_radius_pixels)
                    local_a = geometry_a.intersection(context)
                    local_b = geometry_b.intersection(context)
                    if local_a.is_empty or local_b.is_empty:
                        continue
                    ...
                    if distance > pixel_size * config.door_supported_gap_pixels:
                        continue
                    bridge_geometry: Any = LineString([(point_a.x, point_a.y), (point_b.x, point_b.y)]) if distance > 1e-09 else point_a
                    evidence_distance = float(door_geometry.distance(bridge_geometry))
                    if evidence_distance > pixel_size * context_radius_pixels:
                        continue
                    source = str(evidence.get('evidence_source') or 'door_geometry')
                    confidence = float(evidence.get('confidence') or 0.93)
                    rows.append({'floor_id': floor_id, 'component_a': component_a, 'component_b': component_b, 'geometry_a': geometry_a, 'geometry_b': geometry_b, 'point_a': point_a, 'point_b': point_b, 'distance': distance, 'evidence_source': source, 'evidence_distance': evidence_distance, 'confidence': confidence, 'door_evidence_index': door_index, 'door_source_handle': str(evidence.get('source_handle') or ''), 'target_count_a': int(used_components[component_a]), 'target_count_b': int(used_components[component_b])})
                if (not rows or rows[-1].get('component_a') != component_a or rows[-1].get('component_b') != component_b) and not config.require_confirmed_door_evidence:
                    distance = float(geometry_a.distance(geometry_b))
                    if distance <= pixel_size * config.geometric_gap_pixels:
                        point_a, point_b = nearest_points(geometry_a, geometry_b)
                        rows.append({'floor_id': floor_id, 'component_a': component_a, 'component_b': component_b, 'geometry_a': geometry_a, 'geometry_b': geometry_b, 'point_a': point_a, 'point_b': point_b, 'distance': distance, 'evidence_source': 'geometric_gap', 'evidence_distance': None, 'confidence': 0.55, 'target_count_a': int(used_components[component_a]), 'target_count_b': int(used_components[component_b])})
        rows.sort(key=lambda row: (-float(row['confidence']), row['distance'], -(row['target_count_a'] + row['target_count_b'])))
        return rows
    ...
    def _add_connector_portals(graph: GraphBuilder, targets: list[dict[str, Any]], components: dict[str, list[tuple[str, Polygon]]], pixel_size_by_floor: dict[str, float], opening_by_floor: dict[str, Any], config: RefinementConfig) -> list[dict[str, Any]]:
        ...
        for floor_id, used_components in sorted(used_by_floor.items()):
            ...
            transit_components = Counter(
                {
                    component_id: int(used_components.get(component_id, 0))
                    for component_id in geometry_by_id
                    if node_ids_by_free.get((floor_id, component_id))
                }
            )
            candidates = _connector_candidates(floor_id, transit_components, geometry_by_id, opening_by_floor.get(floor_id), pixel_size_by_floor[floor_id], config)
            component_by_node, _ = _graph_node_groups(graph)
    ...
    def refine_run(run_dir: Path, output_dir: Path | None=None, config: RefinementConfig | None=None, source_dxf: Path | str | None=None) -> dict[str, Any]:
        ...
        opening_by_floor, opening_counts = _load_opening_evidence(
            run_dir,
            pixel_size_by_floor,
            source_dxf=source_dxf,
        )
...
_s07_connector = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.connector_portal_refinement',
    _build_s07_connector(),
    aliases=('connector_portal_refinement',),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/dual_graph/effective_free_space.py
# -----------------------------------------------------------------------------
def _build_s07_effective_free():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/dual_graph/effective_free_space.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.dual_graph.effective_free_space'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.dual_graph'
...
_s07_effective_free = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.dual_graph.effective_free_space',
    _build_s07_effective_free(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/dual_graph/physical_access.py
# -----------------------------------------------------------------------------
def _build_s07_physical_access():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/dual_graph/physical_access.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.dual_graph.physical_access'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.dual_graph'
        ...
        for floor_id, floor in sorted((candidate_bundle.get('floors') or {}).items()):
            ...
            for candidate in floor.get('candidates', []) or []:
                ...
                if not target_id or target_id in present_targets:
                    continue
                if result.get('hard_obstacle_certification'):
                    unavailable.append({'floor_id': _text(floor_id), 'target_id': target_id,
                                        'reason': 'hard_wall_mode_forbids_unverified_nearest_anchor_alias'})
                    continue
...
_s07_physical_access = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.dual_graph.physical_access',
    _build_s07_physical_access(),
    aliases=(),
)

# -----------------------------------------------------------------------------
# Migrated implementation: multi_drawing_pipeline.stages.fire_route_core/semantic/physical_metric_closure.py
# -----------------------------------------------------------------------------
def _build_s07_metric_closure():
    __file__ = str(
        __import__("pathlib").Path(globals()["__file__"]).resolve().parents[2]
        / 'multi_drawing_pipeline.stages.fire_route_core/semantic/physical_metric_closure.py'
    )
    __name__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic.physical_metric_closure'
    __package__ = 'multi_drawing_pipeline.stages.fire_route_core.semantic'
    ...
    def _compact_undirected_csr(node_ids: Sequence[str], edges: Sequence[Mapping[str, Any]]) -> tuple[csr_matrix, int]:
        ...
        for edge in edges:
            ...
            length = _finite(edge.get('routing_cost'), _finite(edge.get('length'), math.nan))
...
_s07_metric_closure = _register_embedded_module(
    'multi_drawing_pipeline.stages.fire_route_core.semantic.physical_metric_closure',
...
def run_stage(
    ...
    *,
    ...
    source_graph = Path(
        rule_navigation_graph
        or run / "area_graph_navigation" / "rule_navigation_graph.json"
    ).resolve()
    source_graph = _require(source_graph, "Stage 06 规则物理导航图")
    ...
    if source_graph != graph_path.resolve():
        shutil.copyfile(source_graph, graph_path)
    _write_json(connector_path, {"type": "FeatureCollection", "features": []})
    refinement = {
        "schema_version": 1,
        "accepted": True,
        "policy": "stage06_graph_passthrough_without_door_portal_refinement",
        "door_portal_refinement_enabled": False,
        "independent_raw_cad_collision_enabled": False,
        "force_refinement_ignored": bool(force_refinement),
        "source_graph": str(source_graph),
        "outputs": {
            "graph_json": str(graph_path.resolve()),
            "connector_geojson": str(connector_path.resolve()),
        },
    }
    _write_json(summary_path, refinement)
    graph_path = _require(graph_path, "Stage 06 直通物理导航图")
    connector_path = _require(connector_path, "空 Connector 兼容文件")
    ...
    if hard_index:
        free_space_audit["hard_obstacle_constraint"] = hard_walls.constrain_effective_free_space(
            Path(effective_free), hard_index
        )
    return PhysicalPreparationResult(
        enabled=True,
